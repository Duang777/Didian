...
const (
	onboardingAssistantName = "Didian Helper"
	onboardingIssueTitle    = "Start here: learn Didian with Didian Helper"
	onboardingAgentTemplate = "didian_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Didian questions and runs CLI operations."
...
// Didian Helper agent created by this shim. Pre-v3 desktop submits a
...
// is removed.
const onboardingAssistantInstructions = `You are Didian Helper, the built-in AI assistant for this Didian workspace. Your role is to help any member use Didian better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to Didian.

This is your guided first run. Didian Helper is assigned to this issue and will help you try the core workflow:

1. Read Didian Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Didian Helper when you want it to continue.
...
//
// Creates or reuses one "Didian Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Didian.",
		"",
		"Agents need a runtime before they can execute work. You can still use Didian as a lightweight project-management workspace while you install one.",
		"",
		"## Try Didian first",
		"",
		...
		"",
		"Full guide: https://didian.ai/docs/install-agent-runtime",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Didian daemon:",
		"   didian daemon restart",
		...
		"",
		"When the runtime is connected, you can create Didian Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Didian。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Didian 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://didian.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Didian Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
