...
Didian's supported protocol families but is launched through a team-specific
command.
...
```sh
didian runtime profile set-path <profile-id> --path /abs/path/to/agent
```
