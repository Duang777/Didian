...
var rootCmd = &cobra.Command{
	Use:           "didian",
	Short:         "Didian CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Didian from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("didian {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Didian server URL (env: DIDIAN_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: DIDIAN_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: DIDIAN_DEBUG)")
