.PHONY: help makehelp dev server daemon cli didian build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
POSTGRES_DB ?= didian
POSTGRES_USER ?= didian
POSTGRES_PASSWORD ?= didian
POSTGRES_PORT ?= 5432
PORT := $(or $(BACKEND_PORT),$(API_PORT),$(SERVER_PORT),$(PORT),8080)
FRONTEND_PORT ?= 3000
FRONTEND_ORIGIN ?= http://localhost:$(FRONTEND_PORT)
DIDIAN_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
GOOGLE_REDIRECT_URI ?= $(FRONTEND_ORIGIN)/auth/callback
DIDIAN_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@echo "==> Pulling official Didian images..."
	@if ! $(COMPOSE) -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${DIDIAN_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Didian via Docker Compose..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		echo ""; \
		echo "✓ Didian is running!"; \
		echo "  Frontend: http://localhost:$${FRONTEND_PORT:-3000}"; \
		echo "  Backend:  http://localhost:$${PORT:-8080}"; \
		echo ""; \
		echo "Images: $${DIDIAN_BACKEND_IMAGE:-ghcr.io/didian-ai/didian-backend}:$${DIDIAN_IMAGE_TAG:-latest}"; \
		echo "        $${DIDIAN_WEB_IMAGE:-ghcr.io/didian-ai/didian-web}:$${DIDIAN_IMAGE_TAG:-latest}"; \
		echo ""; \
		echo "Log in: configure RESEND_API_KEY in .env for email codes,"; \
		echo "        or read the generated code from backend logs when Resend is unset."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install didian-ai/tap/didian"; \
		echo "  didian setup self-host"; \
	else \
		echo ""; \
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Didian from the current checkout..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo "Local tags: didian-backend:dev and didian-web:dev."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install didian-ai/tap/didian"; \
		echo "  didian setup self-host"; \
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) didian DIDIAN_ARGS="daemon restart --profile local"

cli: ## Run the didian CLI with ARGS or DIDIAN_ARGS from source
	@$(MAKE) didian DIDIAN_ARGS="$(DIDIAN_ARGS)"

didian: ## Run the didian CLI entrypoint directly from the Go source tree
	cd server && go run -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" ./cmd/didian $(DIDIAN_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/didian ./cmd/didian
