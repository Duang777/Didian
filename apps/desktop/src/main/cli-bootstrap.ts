...
// Desktop prefers the bundled `didian` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/didian-ai/didian/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "didian.exe" : "didian";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `didian-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `didian` binary. If one is already present at
