...
Deploy Didian on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`didian` CLI** and runs the **agent daemon** on their own machine.
...
curl -fsSL https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.sh | bash -s -- --with-server

# 2. Configure CLI, authenticate, and start the daemon
didian setup self-host
```

This installs the `didian` CLI, checks out the latest self-host assets, pulls the official Didian images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `didian-backend:dev` / `didian-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `DIDIAN_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `DIDIAN_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
If you already run a Kubernetes cluster, you can deploy Didian there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/didian-ai/charts/didian` or the source chart at [`deploy/helm/didian/`](deploy/helm/didian/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `didian-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `didian-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `didian-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `didian-config` ConfigMap (rendered from `values.yaml`)

The `didian-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **One release per namespace:** the prebuilt `didian-web` image bakes `REMOTE_API_URL=http://backend:8080` at build time, so the chart ships an ExternalName Service literally named `backend`. Because that name is unprefixed, you can run only one Didian release per namespace, and `helm install` will fail if a `Service/backend` already exists there (pass `--take-ownership`, or use a dedicated namespace). If you build a web image with a patched `REMOTE_API_URL`, set `frontend.compatibility.backendAlias: false` to drop the alias.
...
The chart defaults to `didian.dev.lan` (web) and `api.didian.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  didian.dev.lan api.didian.dev.lan
  ```
...
```bash
kubectl create namespace didian
```

### Step 3 — Create the `didian-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n didian create secret generic didian-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=DIDIAN_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install didian oci://ghcr.io/didian-ai/charts/didian \
...
```bash
helm show values oci://ghcr.io/didian-ai/charts/didian \
...
```bash
helm install didian deploy/helm/didian -n didian
```

Watch the pods come up:

```bash
kubectl -n didian get pods -w
```
...
```bash
curl -H "Host: api.didian.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://didian.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n didian patch secret didian-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n didian rollout restart deploy/didian-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n didian logs -f deploy/didian-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `DIDIAN_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade didian oci://ghcr.io/didian-ai/charts/didian \
    --version <chart-version> \
    -n didian \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n didian patch secret didian-secrets --type=merge \
    -p '{"stringData":{"DIDIAN_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n didian rollout restart deploy/didian-backend
  ```
...
```bash
didian setup self-host \
  --server-url http://api.didian.dev.lan \
  --app-url http://didian.dev.lan
```
...
```bash
kubectl -n didian rollout restart deploy/didian-backend deploy/didian-frontend
```

To upgrade to a specific Didian release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n didian rollback didian
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of MUL-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-MUL-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n didian exec deploy/didian-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.

### Tearing down

```bash
# Remove the workloads but keep the PVCs and the Secret
helm -n didian uninstall didian

# Wipe everything, including PostgreSQL data and uploads
kubectl delete namespace didian
```
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Didian's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Didian Cloud

If you've been self-hosting and want to switch your CLI to [Didian Cloud](https://didian.ai):

```bash
didian setup
```

This reconfigures the CLI for didian.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `DIDIAN_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `didian setup`:
