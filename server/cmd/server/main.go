...
import (
	...
	"github.com/didian-ai/didian/server/internal/scheduler"
...
func main() {
	...
	if os.Getenv("DIDIAN_DEV_VERIFICATION_CODE") != "" {
		if strings.EqualFold(strings.TrimSpace(os.Getenv("APP_ENV")), "production") {
			slog.Warn("DIDIAN_DEV_VERIFICATION_CODE is set but ignored because APP_ENV=production.")
		} else {
			slog.Warn("DIDIAN_DEV_VERIFICATION_CODE is enabled. Use it only for local development or private test instances.")
		}
	}
	...
	// Feature flags: loaded once at startup from DIDIAN_FEATURE_FLAGS_FILE
	...
	go runBrowserMemoryEnrichmentWorker(sweepCtx, h.MemoryEnrichment)
