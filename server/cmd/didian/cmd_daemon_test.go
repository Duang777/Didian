...
func TestPrintDiskUsageOtherRootsHintSuggestsProfilesWithTasks(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("DIDIAN_WORKSPACES_ROOT", "")
	...
	printDiskUsageOtherRootsHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "didian_workspaces"),
	}, "", "")
	...
	if !strings.Contains(got, "didian --profile two-tasks daemon disk-usage") {
		t.Fatalf("hint output = %q, want two-tasks profile command", got)
	}
	if !strings.Contains(got, "didian --profile one-task daemon disk-usage") {
		t.Fatalf("hint output = %q, want one-task profile command", got)
	}
	if !strings.Contains(got, "didian --profile 'space profile' daemon disk-usage") {
		t.Fatalf("hint output = %q, want shell-quoted profile command", got)
	}
	if !strings.Contains(got, "didian daemon disk-usage --all-profiles") {
...
func TestPrintDiskUsageOtherRootsHintFiresWhenCurrentRootNonEmpty(t *testing.T) {
	...
	got := out.String()
	if !strings.Contains(got, "didian --profile desktop-host daemon disk-usage") {
...
func TestPrintDiskUsageOtherRootsHintSuggestsDefaultFromNamedProfile(t *testing.T) {
	...
	var out bytes.Buffer
	printDiskUsageOtherRootsHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "didian_workspaces_named"),
	}, "named", "")

	got := out.String()
	if !strings.Contains(got, "didian daemon disk-usage  #") {
...
func TestEnumerateDiskUsageRoots(t *testing.T) {
	...
	// Two profiles configured under ~/.didian/profiles, but only one has its
	...
	if roots[0].Profile != "" || roots[0].Root != filepath.Join(home, "didian_workspaces") {
		t.Fatalf("roots[0] = %+v, want default root first", roots[0])
	}
	if roots[1].Profile != "desktop-host" || roots[1].Root != filepath.Join(home, "didian_workspaces_desktop-host") {
...
func TestPrintAggregateDiskUsageShowsRootsAndGrandTotal(t *testing.T) {
	agg := daemon.AggregateDiskUsageReport{
		Roots: []daemon.RootDiskUsage{
			{Profile: "", Report: daemon.DiskUsageReport{
				WorkspacesRoot: "/home/u/didian_workspaces",
			...
			{Profile: "desktop-host", Report: daemon.DiskUsageReport{
				WorkspacesRoot: "/home/u/didian_workspaces_desktop-host",
	...
	if !strings.Contains(got, "/home/u/didian_workspaces_desktop-host") {
...
func mkdirProfile(t *testing.T, home, profile string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(home, ".didian", "profiles", profile), 0o755); err != nil {
		t.Fatal(err)
	}
}
...
func writeDiskUsageTaskFile(t *testing.T, home, profile, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "didian_workspaces_"+profile, workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}

func writeDefaultDiskUsageTaskFile(t *testing.T, home, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "didian_workspaces", workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}
