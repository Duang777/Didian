...
description: 在自己的服务器或本机用 Docker 把 Didian 跑起来（也可以在 Kubernetes 上用 Helm）。约 10 分钟。
---

import { Callout } from "fumadocs-ui/components/callout";

这一页带你用 Docker 把 Didian 的**服务器**（后端 + 前端 + PostgreSQL）跑在自己的机器或服务器上。走完这一篇你的数据就完全在自己手里——包括 [工作区](/workspaces)、[issue](/issues)、[评论](/comments)、[智能体](/agents) 配置。
...
2. 拉取官方 Docker 镜像（PostgreSQL、Didian backend、Didian frontend）
...
**`docker-compose.selfhost.yml` 默认把 `APP_ENV` 设成 `production`**，并让 `DIDIAN_DEV_VERIFICATION_CODE` 为空，所以公网实例默认没有固定验证码。

只在本地或私有测试自动化里设置 `DIDIAN_DEV_VERIFICATION_CODE`。如果在 `APP_ENV` 非 production 时启用了固定验证码，任何能请求验证码的人都能用这个固定值登录。详见 [登录与注册配置 → 固定本地测试验证码](/auth-setup#固定本地测试验证码)。

公网部署前一定检查 `.env` 里 `APP_ENV=production`，且 `DIDIAN_DEV_VERIFICATION_CODE` 为空。
</Callout>
...
- 不要直接使用 `888888`；只有在非 production 私有实例上显式设置 `DIDIAN_DEV_VERIFICATION_CODE=888888` 后它才会生效
- 登录后创建第一个工作区

## 5. 连接命令行工具到你自己的 server

命令行装法和 [Cloud 快速上手 → 2. 装命令行工具](/cloud-quickstart#2-装-didian-命令行工具) 一样——Homebrew / 脚本 / PowerShell 任选。
...
<Callout type="info">
更习惯用环境变量？省略对应 flag 时，`setup self-host` 会读取 `DIDIAN_SERVER_URL` 和 `DIDIAN_APP_URL`（同时设置时 flag 优先）。`DIDIAN_SERVER_URL` 也接受[环境变量](/environment-variables)里那种 `ws://…/ws` 的 daemon 写法，并自动归一化为 HTTP 地址。
</Callout>
...
代理起好之后，记得在 server 的 `.env` 里把 `FRONTEND_ORIGIN` 设成 `https://didian.example.com` 并重启后端，否则 WebSocket 的 origin 校验会把浏览器拒掉（见 [故障排查 → WebSocket 连不上](/troubleshooting#websocket-连不上)）。
...
完整参考（含运维注意事项和 Kubernetes 部署形态）见仓库的 [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/didian-ai/didian/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup)。

## Kubernetes 部署（替代方案）

如果你已经在跑 Kubernetes 集群，仓库里也带了一个 Helm chart，路径 `deploy/helm/didian/`。它就是 k8s 版的 `make selfhost`——一样的 backend 镜像、frontend 镜像、`pgvector/pgvector:pg17` Postgres，封装成 Deployment / Service / Ingress，再加上一个由 `values.yaml` 渲染出来的 `ConfigMap`。这套 chart 是按照 k3s + Traefik + `local-path` 写的，集群里只要有 Ingress controller 和默认的 `ReadWriteOnce` StorageClass 就能跑，其他类型的集群稍微改一改也能用。

这个 chart **不会模板化任何敏感值**。它通过 name 引用一个叫 `didian-secrets` 的 Secret，所以真实的 JWT / DB / Resend / Google 密钥永远不用进 git，也不用进 `values.yaml`。先用 kubectl 一次性把命名空间和 Secret 建好：
...
默认主机名是 `didian.dev.lan`（web）和 `api.didian.dev.lan`（backend）。把它们加进 `/etc/hosts`（或者本地 DNS），指向任意一个 Ingress 可达的节点 IP 就行。要换主机名，就把 `deploy/helm/didian/values.yaml` 复制一份，改掉 `ingress.frontend.host` / `ingress.backend.host`，再把 `backend.config.appUrl` / `frontendOrigin` / `localUploadBaseUrl` / `googleRedirectUri` 改成相应的地址，然后 `helm install ... -f my-values.yaml`。
...
然后浏览器打开 `http://didian.dev.lan`，回到上面的 [第 4 步——首次登录](#4-首次登录--创建工作区) 继续。命令行连到你的 Ingress 主机：
...
只想拉最新镜像、不动 chart：`kubectl -n didian rollout restart deploy/didian-backend deploy/didian-frontend`。要锁到某个 Didian 版本，就在 values 文件里设 `images.backend.tag` / `images.frontend.tag`，再 `helm upgrade`。`helm -n didian uninstall didian` 只删工作负载，PVC 和 Secret 都保留；`kubectl delete namespace didian` 才会全清。

完整参考——三种登录方式、为了绕过 web 镜像 build-time 写死的 `REMOTE_API_URL` 而加的 `backend` ExternalName 别名、资源限制、TLS——都在仓库的 [`SELF_HOSTING.md`](https://github.com/didian-ai/didian/blob/main/SELF_HOSTING.md#kubernetes-deployment-alternative)。
...
- [桌面应用](/desktop-app) —— 可以通过 `~/.didian/desktop.json` 连接 Desktop；Web 前端 + CLI 仍然是最快的自部署路径
