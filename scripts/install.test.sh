...
_setup_sandbox() {
  ...
  cat >"$payload_dir/didian" <<'STUB'
#!/usr/bin/env bash
echo "didian v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/didian"
  tar -czf "$tmp/didian.tar.gz" -C "$payload_dir" didian

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/didian-ai/didian/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    DIDIAN_BIN_DIR="$tmp/install-bin" \
    DIDIAN_TEST_ARCHIVE="$tmp/didian.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/didian" ]]; then
    echo "expected fallback binary at $tmp/install-bin/didian" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
