---
title: 环境变量
description: self-host Didian 服务器需要配置的环境变量清单。
---

import { Callout } from "fumadocs-ui/components/callout";

Didian 的 [自部署](/self-host-quickstart) 服务器启动时从环境变量读取配置——数据库、登录、邮件、存储、注册白名单都在这里配。这一页按用途分组给完整清单：每组说清楚**不设会怎样**、**生产必须设哪几个**。Auth 相关那几个怎么真正配见 [登录与注册配置](/auth-setup)。
...
| 环境变量 | 默认值 | 生产必须设？ |
|---|---|---|
| `DATABASE_URL` | `postgres://didian:didian@localhost:5432/didian?sslmode=disable` | **是** |
| `PORT` | `8080` | 否（除非换端口）|
| `JWT_SECRET` | `didian-dev-secret-change-in-production` | **是**（默认值不安全）|
| `APP_ENV` | 空 | **是**（必须 `production`）|
| `FRONTEND_ORIGIN` | 空 | **是**（self-host 要填你自己的域名）|
| `DIDIAN_DEV_VERIFICATION_CODE` | 空 | 否（生产必须保持为空）|

<Callout type="warning">
**生产环境保持 `DIDIAN_DEV_VERIFICATION_CODE` 为空。** 固定本地测试验证码默认关闭；如果你设置 `DIDIAN_DEV_VERIFICATION_CODE=888888`，在 `APP_ENV` 非 production 时，任何能请求验证码的人都能用这个固定值登录。`APP_ENV=production` 时该快捷码会被忽略。
</Callout>
...
Didian 支持两种邮件发送通道——[Resend](https://resend.com/) 适合公网部署，SMTP relay 适合内网/自部署。同时设置时 `SMTP_HOST` 优先级高于 `RESEND_API_KEY`。
...
| `RESEND_FROM_EMAIL` | `noreply@didian.ai` | 发件地址（必须是 Resend 账号已验证的域名；走 SMTP 时同时作为 `From:` 头）|
...
Didian 存储用户上传的附件（评论里的图片、文件等）。**优先走 S3**；不配 S3 就回落本地磁盘。
...
**邀请流程本身不检查 signup 白名单**——但被邀请人必须先能**登录**才能接受邀请。如果对方已经有 Didian 账号（比如在其他工作区注册过），可以直接接受，不受白名单影响；**如果对方还没注册过**，他们登录的第一步（发送验证码）仍然会过白名单检查，被 `ALLOW_SIGNUP=false` 或 `ALLOWED_EMAILS` / `ALLOWED_EMAIL_DOMAINS` 拒绝的邮箱**无法完成注册，也就没法接受邀请**。
...
如果还想让被邀请的新用户能完成首次注册，保留 `ALLOW_SIGNUP=true`（必要时用 `ALLOWED_EMAIL_DOMAINS` / `ALLOWED_EMAILS` 限定可注册邮箱），只把 `DISABLE_WORKSPACE_CREATION=true` 打开即可；如果同时设 `ALLOW_SIGNUP=false`，连有 pending invite 的邮箱都无法完成首次注册，仅适合所有成员都已有 Didian 账号的实例。
...
这里的 `RATE_LIMIT_TRUSTED_PROXIES` 和 `DIDIAN_TRUSTED_PROXIES` **不是同一个**变量——后者控制的是 autopilot webhook 端点（`/api/webhooks/autopilots/{token}`）的限流器。两个限流器各自读各自的列表，部署在代理后面的实例需要两个都配上。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `DIDIAN_SERVER_URL` | `ws://localhost:8080/ws` | server 地址（self-host 换成你的域名）|
| `DIDIAN_DAEMON_HEARTBEAT_INTERVAL` | `15s` | 心跳频率 |
| `DIDIAN_DAEMON_POLL_INTERVAL` | `3s` | 任务轮询频率 |
| `DIDIAN_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 并发任务上限 |
| `DIDIAN_AGENT_TIMEOUT` | `0` | 单次任务的绝对墙钟上限；`0` = 不设上限，任务只受看门狗约束（活跃任务不会因为跑得久被杀）。想要硬性成本/资源天花板时再设一个正值 |
| `DIDIAN_AGENT_IDLE_WATCHDOG` | `30m` | 空闲看门狗：backend 持续静默（无消息、消息队列为空、且没有工具在途）这么久就 force-stop。`0` = 关闭整套看门狗 |
| `DIDIAN_AGENT_TOOL_WATCHDOG` | `2h` | 工具在途时的静默上限：某个工具调用发出后长时间无任何输出（疑似卡死的子进程）这么久就 force-stop。`0` = 关闭该兜底（在途工具永不被停）|
| `DIDIAN_<PROVIDER>_PATH` | 对应 CLI 名 | 各 AI 编程工具的可执行文件路径（如 `DIDIAN_CLAUDE_PATH`）|
| `DIDIAN_<PROVIDER>_MODEL` | 空 | 各 AI 编程工具的默认模型 |
| `DIDIAN_<PROVIDER>_ARGS` | 空 | 守护进程级的默认 CLI 参数，作用于该后端的每个任务，并排在各智能体自身的 `custom_args` 之前。支持 `DIDIAN_CLAUDE_ARGS`、`DIDIAN_CODEX_ARGS`、`DIDIAN_CODEBUDDY_ARGS` |

完整解释每个参数对守护进程行为的影响，见 [守护进程与运行时](/daemon-runtimes)。

### 默认智能体参数（`DIDIAN_<PROVIDER>_ARGS`）
...
- **解析：** 取值按 POSIX shell-word 规则切分，因此引号可用——`DIDIAN_CLAUDE_ARGS='--append-system-prompt "multi word"'` 会解析为两个 token。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FRONTEND_ORIGIN` | 空 | 前端地址。邀请邮件里的链接、CORS 白名单、cookie domain 都从这里推导。邮件链接在不设时会 fallback 到托管版域名 `https://app.didian.ai`——self-host 必须显式填 |
| `DIDIAN_APP_URL` | 空 | CLI 登录流程使用的前端 URL。Web UI 也会用它显示带你自己 app domain 的 self-host 守护进程 setup 命令；同源部署中，如果 `DIDIAN_PUBLIC_URL` 未设置，它也会作为守护进程的 `server_url` |
| `DIDIAN_PUBLIC_URL` | 空 | 公开 API URL，不带结尾 slash。用于 autopilot webhook URL，也会被 Web UI 用作守护进程的 `server_url` |
...
<Callout type="warning">
**`FRONTEND_ORIGIN` 不设就有两个静默失败**：（1）邀请邮件里的链接指向 `https://app.didian.ai`（托管版的域名），用户点了跳不回你的 self-host 实例；（2）WebSocket 连接的 Origin 校验回落到 `localhost:3000 / 5173 / 5174`，生产部署的 WebSocket 全部被拒，前端看起来「实时更新不工作」。
</Callout>
...
- `/api/webhooks/github` 直接返回 **`503 github webhooks not configured`**——secret 没配置时 Didian 拒绝处理任何 webhook 事件，而不是把所有签名当 valid

**可选 `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY` 未设时：**

- 安装完成后，连接卡片会先短暂显示 `已连接到 unknown`。等 GitHub 的 `installation.created` webhook 到达（通常几秒内），Didian 会把 row 刷成真实的组织/用户名，并通过 realtime 推送让正在打开的 Settings → GitHub 页面无需手动刷新即可更新。
...
默认上报到 Didian 官方 PostHog 实例。不想上报就把 `ANALYTICS_DISABLED=true`。
...
- [守护进程与运行时](/daemon-runtimes) —— `DIDIAN_DAEMON_*` 参数的行为含义
