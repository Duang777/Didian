...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/didian-ai/didian","ref":"release/v2","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".didian", "project", "resources.json")
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".didian", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestPrepareWithRepoContext(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".didian" && name != "CLAUDE.md" && name != ".claude" {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"didian autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `didian issue get") {
...
// collision-free duplicates (issue-review, issue-review-didian,
// issue-review-didian-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -didian duplicates", names)
	}
...
// next refresh dodges to issue-review-didian. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -didian duplicate", names)
	}
...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Didian Agent Runtime",
		"didian issue get",
		"didian issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"didian issue comment add --help",
		"didian squad member set-role <squad-id>",
	} {
		if !strings.Contains(s, want) {
	...
	for _, banned := range []string{
		"didian issue list [--status",
		"didian issue label list",
		"didian issue subscriber list",
		"didian label list",
		"didian workspace member list",
		"didian agent list",
		"didian squad list",
		"didian issue runs",
		"didian issue run-messages",
		"didian attachment download",
		"didian autopilot list",
		"didian autopilot create",
		"didian autopilot update",
		"didian autopilot trigger",
		"didian autopilot delete",
		"didian project get",
		"didian project resource list",
		"didian issue assign",
		"didian issue label add",
		"didian issue label remove",
		"didian issue subscriber add",
		"didian issue subscriber remove",
		"didian issue comment delete",
		"didian label create",
	} {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Didian Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "didian issue get") {
		t.Error("should reference didian CLI even without skills")
	}
...
func TestPrepareWithRepoContextOpencode(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".didian" && name != "AGENTS.md" {
			t.Errorf("unexpected entry in workdir: %s", name)
		}
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `didian issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"didian issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `didian issue comment add`",
...
// heredoc on `didian issue create`, the flags were silently swallowed into
...
// non-ASCII as `?` before reaching `didian.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		didianManagedBeginMarker,
		didianMultiAgentBeginMarker,
		didianMemoryFeatureBeginMarker,
		didianMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, didianManagedBeginMarker) || !strings.Contains(s, didianManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertDidianManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), didianManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.didian.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, didianManagedBeginMarker)
	endIdx := strings.Index(s, didianManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.didian]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.didian]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.didian content")
	}
	...
	if n := strings.Count(string(data2), didianManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.didian], got:\n%s", s)
	}
	if strings.Count(s, didianManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), didianManagedBeginMarker) {
		t.Error("reused config.toml missing didian-managed block")
	}
}
...
// inside a Didian task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"didian issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
		"didian issue comment list " + issueID + " --recent 10 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "didian issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigCommentTriggerResumedNoDeltaRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"didian issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"didian issue comment list issue-1 --recent 10 --output json",
	...
	for _, banned := range []string{
		"didian issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"didian issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"didian issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `didian issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"didian issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"didian issue metadata set",
				"didian issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "didian issue metadata list issue-md-codex --output json") {
