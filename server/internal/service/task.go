...
import (
	...
	"github.com/didian-ai/didian/server/pkg/redact"
...
func (s *TaskService) CompleteTask(ctx context.Context, taskID pgtype.UUID, result []byte, sessionID, workDir string) (*db.AgentTaskQueue, error) {
	...
	if err := s.runInTx(ctx, func(qtx *db.Queries) error {
		...
		if bm, ok := s.parseBrowserMemoryEnrichmentContext(t); ok {
			if err := s.completeBrowserMemoryEnrichment(ctx, qtx, bm, result); err != nil {
				if !errors.Is(err, errBrowserMemoryInvalidOutput) {
					return err
				}
				if failErr := s.failBrowserMemoryEnrichment(ctx, qtx, bm, err.Error()); failErr != nil {
					return failErr
				}
				slog.Warn("browser memory enrichment output rejected",
...
func (s *TaskService) FailTask(ctx context.Context, taskID pgtype.UUID, errMsg, sessionID, workDir, failureReason string) (*db.AgentTaskQueue, error) {
	...
	if err := s.runInTx(ctx, func(qtx *db.Queries) error {
		...
		if bm, ok := s.parseBrowserMemoryEnrichmentContext(t); ok {
			if err := s.failBrowserMemoryEnrichment(ctx, qtx, bm, errMsg); err != nil {
				return err
			}
		}
