...
description: Deploy Didian on your own infrastructure.
---

## Architecture Overview

Didian has three components:
...
Each user who wants to run AI agents locally also installs the **`didian` CLI** and runs the **agent daemon** on their own machine.
...
This installs the CLI, checks out the latest self-host assets, pulls the official Didian images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install didian-ai/tap/didian`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `didian config set server_url http://localhost:8080 && didian config set app_url http://localhost:3000 && didian login && didian daemon start`
</Callout>
...
Full reference (audit table semantics, advisory lock 4246, the standalone backfill command, flag descriptions, the migration auto-hook) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/didian-ai/didian/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string. Keep the password segment in sync with `POSTGRES_PASSWORD`. | `postgres://didian:<postgres-password>@localhost:5432/didian?sslmode=disable` |
...
Didian uses email-based magic link authentication via [Resend](https://resend.com).
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`didian`/`didian`).
