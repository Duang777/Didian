...
// loginTokenPrefixes are the token prefixes `didian login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a Didian Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate didian with Didian",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("DIDIAN_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the DIDIAN_DAEMON_PORT
	// signal for subprocesses that lost DIDIAN_AGENT_ID / DIDIAN_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"DIDIAN_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'didian setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `didian login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `didian login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `didian login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'didian login' to authenticate.")
		return nil
	}
...
<title>Didian — Authenticated</title>
<style>
