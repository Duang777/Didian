#!/usr/bin/env bash
# Didian installer — installs the CLI and optionally provisions a self-host server.
#
...
#
# After installation, run `didian setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/didian-ai/didian.git"
REPO_WEB_URL="https://github.com/didian-ai/didian"  # without .git, for GitHub web APIs
INSTALL_DIR="${DIDIAN_INSTALL_DIR:-$HOME/.didian/server}"
BREW_PACKAGE="didian-ai/tap/didian"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Didian supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing Didian CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap didian-ai/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Didian CLI already installed via Homebrew"
    else
      warn "Failed to install didian via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Didian CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Didian CLI from GitHub Releases..."
  ...
  local url="https://github.com/didian-ai/didian/releases/download/${latest}/didian-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/didian.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/didian.tar.gz" -C "$tmp_dir" didian

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with DIDIAN_BIN_DIR.
  local bin_dir="${DIDIAN_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/didian" "$bin_dir/didian"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/didian" "$bin_dir/didian"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/didian" "$bin_dir/didian"
    chmod +x "$bin_dir/didian"
  ...
  rm -rf "$tmp_dir"
  ok "Didian CLI installed to $bin_dir/didian"
}
...
get_selfhost_ref() {
  if [ -n "${DIDIAN_SELFHOST_REF:-}" ]; then
    printf '%s' "$DIDIAN_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Didian CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Didian CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Didian CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists didian; then
    local current_ver
    # `didian version` outputs "didian 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(didian version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Didian CLI is up to date ($current_ver)"
      return 0
    fi

    info "Didian CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(didian version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "Didian CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists didian; then
    fail "CLI installed but 'didian' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Didian self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning Didian repository..."
  ...
  info "Pulling official Didian images..."
  pull_official_selfhost_images
  info "Starting Didian services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Didian — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Didian CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}didian setup${RESET}                # Connect to Didian Cloud (didian.ai)\n"
  printf "     ${CYAN}didian setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Didian — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Didian server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}didian setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Didian installation found at $INSTALL_DIR"
  fi

  if command_exists didian; then
    didian daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Didian CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  DIDIAN_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.didian/server)"
        echo "  DIDIAN_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  DIDIAN_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'didian setup' to configure your environment."
        exit 0
        ;;
