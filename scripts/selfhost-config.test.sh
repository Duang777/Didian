...
require_config "$config" 'DIDIAN_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "DIDIAN_APP_URL=${DIDIAN_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "DIDIAN_SERVER_URL=${DIDIAN_SERVER_URL}" \
...
require_env "$local_env" 'DIDIAN_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'DIDIAN_SERVER_URL=ws://localhost:9100/ws'
