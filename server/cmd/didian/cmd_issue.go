...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `didian issue status`.",
	Args: exactArgs(1),
...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Didian-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Didian-Next-Before-Id response header; must be paired with --before.")
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("DIDIAN_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse DIDIAN_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	// Quick-create stamp: when the daemon sets DIDIAN_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `didian issue create` call
	...
	if taskID := os.Getenv("DIDIAN_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `didian issue status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Didian-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Didian-Next-Before-Id"); nbid != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set DIDIAN_WORKSPACE_ID")
	}
...
// from `didian workspace member list --output json`) should use this instead of
