...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The didian- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "didian-") {
				t.Errorf("skill name %q must carry the didian- prefix", skill.Name)
			}
...
// must declare user-invocable:false and fence itself to the didian CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "didian-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(didian *)" {
		t.Errorf("allowed-tools = %q, want Bash(didian *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "didian-working-on-issues")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "Bash(didian *)") {
		t.Errorf("allowed-tools = %q, want access to the Didian CLI", got)
	}
	...
	mustContain := []string{
		...
		"open or update a PR before posting the final Didian issue comment",
	...
	mustNotContain := []string{
		...
		"didian issue get <issue-id> --output json",
		"didian issue metadata list <issue-id> --output json",
		"didian issue comment list <issue-id> --thread <trigger-comment-id>",
		"didian issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "didian-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustNotContain := []string{
		"didian agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "didian-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"didian agent create --name <name> --runtime-id <runtime-id>",
		...
		"didian agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"didian agent skills list <agent-id> --output json",
		"didian agent get <agent-id> --output json",
		"255",
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "didian-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "didian-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"didian autopilot trigger-add <autopilot-id> --kind schedule",
		"didian autopilot trigger <autopilot-id> --output json",
		"Do not run `trigger`",
		"webhook tokens",
		"{{date}}",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "didian-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestProjectsAndResourcesSkillCoversDurableContext(t *testing.T) {
	skill, ok := findSkill(t, "didian-projects-and-resources")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"didian project resource list <project-id> --output json",
		"didian project resource add <project-id> --type github_repo --url <github-url> --output json",
		"didian project resource add <project-id> --type github_repo --url <github-url> --ref <branch-or-sha> --output json",
		"didian project resource add <project-id> --type local_directory",
