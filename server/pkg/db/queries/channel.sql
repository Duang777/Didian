...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Didian
...
-- the conflict update to the SAME Didian workspace: a team already owned by a
...
-- old catch-all "connected to a different Didian workspace". Meant to be read
...
-- to a Didian user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on didian_user_id
...
INSERT INTO channel_user_binding (
    workspace_id, didian_user_id, installation_id,
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.didian_user_id = EXCLUDED.didian_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Didian
...
-- bound to ANOTHER installation in the SAME Didian workspace + SAME Slack team,
...
-- different Didian workspaces, and a user may hold different Didian accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND didian_user_id = $2;
