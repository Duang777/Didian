...
func TestLarkOutcomeReplierUsesAppURLForWebLinks(t *testing.T) {
	...
	if got := stub.bindingCalls[0].BindURL; got != "https://app.didian.test/lark/bind?token=token+with+space" {
	...
	if !strings.Contains(stub.textOut[0].Text, "https://app.didian.test/issues/MUL-42") {
...
func TestLarkOutcomeReplierIssueCreatedSendsConfirmation(t *testing.T) {
	...
	if !strings.Contains(got.Text, "https://didian.test/issues/MUL-42") {
		t.Errorf("text should embed the deep link back to Didian; got %q", got.Text)
	}
