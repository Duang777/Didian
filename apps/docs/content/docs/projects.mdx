...
A **project** in Didian is a container for related [issues](/issues). Use it when a body of work is bigger than one issue but smaller than a full workspace — a launch, a migration, a feature with multiple parts, an investigation that branches into several threads.
...
Each project has a **Resources** section where you attach GitHub repositories. Once attached, any [agent](/agents) assigned to issues in this project can read and write to those repos when executing tasks — Didian passes the repo URLs as context to the [daemon](/daemon-runtimes).
...
- [How Didian works](/how-didian-works) — the broader picture
