...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_comment_done","update":{"type":"ToolCall","toolCallId":"tc-comment","name":"Shell","status":"pending","parameters":{"command":"didian issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestKiroIssueCommentAddCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{"didian issue comment add issue-1 --content-file ./reply.md", true},
		{"./didian issue comment add issue-1 --content-file ./reply.md", true},
		{"/usr/local/bin/didian issue comment add issue-1 --content-file ./reply.md", true},
		{"DIDIAN_TOKEN=x didian issue comment add issue-1 --content-file ./reply.md", true},
		{"FOO=1 BAR=2 ./didian issue comment add issue-1", true},
		{`sh -c "didian issue comment add issue-1 --content-file ./reply.md"`, true},
		{`bash -c 'didian issue comment add issue-1'`, true},
		{`/bin/sh -c "didian issue comment add issue-1"`, true},
		{"didian issue get issue-1", false},
		{"echo didian issue comment add issue-1", false},
		{`sh -c "echo didian issue comment add issue-1"`, false},
		{"FOO=bar", false},
		{"", false},
	}
	for _, tt := range tests {
...
func TestKiroBackendInvokesACPWithTrustAllTools(t *testing.T) {
	...
	session, err := backend.Execute(ctx, "prompt-ignored", ExecOptions{
		Model:      "bogus-model",
		Timeout:    5 * time.Second,
		CustomArgs: []string{"acp", "--trust-tools", "shell", "-a", "--agent", "didian"},
	})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	...
	if strings.Join(lines, "\n") != strings.Join([]string{"acp", "--trust-all-tools", "--agent", "didian"}, "\n") {
