...
// for the Didian Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: didian-agent <github@didian.ai>"
...
// path: a bare cache that already carries the Didian prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: didian-agent") {
...
// prepare-commit-msg hook that does NOT include the didianHookMarker
// sentinel — only the older `# Installed by the Didian daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no didianHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: didian-agent") {
...
// without the Didian marker (e.g. one a user added manually) must be left
