...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Didian-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN DIDIAN-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END DIDIAN-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Didian runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Didian marker
...
//
//   - User content carries a stray `<!-- END DIDIAN-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Didian marker block from the runtime
...
// of Didian runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Didian task would pick up
...
// agent about the Didian runtime environment and available CLI tools.
//
