...
import { createPersistStorage, defaultStorage } from "@didian/core/platform";
import { createSafeId } from "@didian/core/utils";
...
/** Default entry point for a workspace — its Mission list. */
      ...
      switchWorkspace(slug, openPath) {
        ...
        if (!existing) {
          ...
          const seedPath =
            desiredPath && sanitizeTabPath(desiredPath) === desiredPath
              ? desiredPath
              : defaultPathFor(slug);
          const tab = makeTab(seedPath, "Missions", resolveRouteIcon(seedPath));
          set({
        ...
        if (desiredPath) {
          ...
          if (clean) {
            ...
            const tab = makeTab(clean, "Missions", resolveRouteIcon(clean));
            set({
              activeWorkspaceSlug: slug,
              byWorkspace: {
                ...byWorkspace,
