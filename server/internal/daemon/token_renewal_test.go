...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "didian login") {
		t.Fatalf("401 warning must tell the user to run 'didian login', got: %s", out)
	}
}
...
// and the operator would never see a "run didian login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "didian login") {
		t.Fatalf("expected the actionable 'run didian login' hint in the WARN, got: %s", out)
	}
}
