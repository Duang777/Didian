---
title: 移动 App（iOS）
description: 在自己的 iPhone 上自助 build 开源版 Didian iOS app —— 暂未上 App Store。
---

import { Callout } from "fumadocs-ui/components/callout";

Didian iOS 客户端开源,跟 web、desktop、后端一起放在[主仓库](https://github.com/didian-ai/didian)里。目前没上 App Store —— 在那之前,想用的人自己从源码 build 一份。首次 build 约 10–20 分钟,之后每次约 2 分钟,连接的是 [didian.ai](https://didian.ai) 同一个后端,所以你现有账号直接能登。

<Callout type="info">
本页是给**个人使用者**看的。如果你是要开发这个 app,请看仓库里的 [`apps/mobile/README.md`](https://github.com/didian-ai/didian/blob/main/apps/mobile/README.md) —— 那里覆盖 dev / staging 变体和完整脚本表。
</Callout>
...
暂时没有自动更新。Didian 代码库前进时,你 pull 然后重 build:
...
iOS app 还在快速迭代 —— 团队目前更倾向于"先发再改",而不是 App Store 审核周期。下一步比较可能是 TestFlight 内测,然后才是正式上架。在那之前,上面的自助 build 是 iOS 上用 Didian 的唯一方式。

想第一时间知道 TestFlight 开放的话,watch 一下 [GitHub 仓库](https://github.com/didian-ai/didian)。

## 排错

**"No matching provisioning profiles found"** —— Xcode 拒绝用你的 Apple ID 签默认的 `ai.didian.mobile`。比较罕见,如果有人在 Apple Developer Portal 抢注了这个前缀就会出现。换一个你控制的反向域名(`com.yourname.didian` 就够),export 后重跑:
...
**App 连不上后端** —— 确认 `apps/mobile/.env.production` 没动过(默认值 `EXPO_PUBLIC_API_URL=https://api.didian.ai`)。如果你改过,用 `git checkout apps/mobile/.env.production` 还原。
