...
import { useWelcomeStore } from "@didian/core/onboarding";
...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates a Helper agent then opens a blocking modal with starter cards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/welcome to Didian/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("Didian Helper");
      expect(agentArgs.instructions).toContain("Didian Helper");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce Didian to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what Didian can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("reuses an existing Didian Helper agent instead of creating duplicates", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "Didian Helper",
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce Didian to me"));
      fireEvent.click(
        screen.getByText("Show me what Didian can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce Didian to me",
        "Show me what Didian can do for me — as slides",
      ]);
    ...
    it("uses Korean persisted Helper and starter issue artifacts under ko locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Didian를 간단히 소개해 주세요"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Didian 사용 어시스턴트");
      expect(agentArgs.instructions).toContain(
        "당신은 이 Didian 워크스페이스에 내장된 AI 어시스턴트",
      );

      fireEvent.click(screen.getByText("Didian를 간단히 소개해 주세요"));
      fireEvent.click(
      ...
      expect(issueArgs.title).toBe("Didian를 간단히 소개해 주세요");
      expect(issueArgs.description).toContain(
        "Didian를 1-2문단으로 간단히 소개해 주세요",
      );
    });
    ...
    it("uses Japanese persisted Helper and starter issue artifacts under ja locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Didian を簡単に紹介してください"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Didian の使い方アシスタント");
      expect(agentArgs.instructions).toContain(
        "あなたは Didian Helper、この Didian ワークスペースに組み込まれた AI アシスタント",
      );

      fireEvent.click(screen.getByText("Didian を簡単に紹介してください"));
      fireEvent.click(
      ...
      await waitFor(() => expect(mockCreateIssue).toHaveBeenCalledTimes(1));
      const [issueArgs] = mockCreateIssue.mock.calls[0]!;
      expect(issueArgs.title).toBe("Didian を簡単に紹介してください");
      expect(issueArgs.description).toContain(
        "Didian を1〜2段落で簡単に紹介してください",
      );
    });
  });
  ...
  describe("skip path", () => {
    it("provisions install-runtime → agent-guide → follow-up comment, then opens the celebration Modal", async () => {
      ...
      expect(secondCall![0].title).toBe(
        "Step 2 — Create your first Didian Agent",
      );
    ...
    it("silently dismisses without showing the Modal when provisioning fails", async () => {
      ...
      expect(screen.queryByText(/Welcome to Didian/i)).not.toBeInTheDocument();
    });
    ...
    it("uses Korean persisted skip-path issue and comment artifacts under ko locale", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Didian에 오신 것을 환영합니다/i)).toBeInTheDocument();
      });
    ...
    it("uses Japanese persisted skip-path issue and comment artifacts under ja locale", async () => {
      ...
      expect(installCall![0].description).toContain("Didian へようこそ。");
      expect(guideCall![0].title).toBe(
        "ステップ2 — 最初の Didian Agent を作成する",
      );
