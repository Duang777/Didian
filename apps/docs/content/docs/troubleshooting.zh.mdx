---
title: 故障排查
description: self-host Didian 常见问题——症状、原因、怎么查、怎么修。
---

import { Callout } from "fumadocs-ui/components/callout";

按症状查问题。每条问题都给**症状 / 可能原因 / 怎么查 / 怎么修**四段。如果你的情况不在下面，到 [GitHub](https://github.com/didian-ai/didian/issues) 提 issue。

## 守护进程连不上服务器

**症状**：[`didian daemon`](/cli) 的 `status` 命令显示 `offline` 或 `connection refused`；服务器日志里没有 `/api/daemon/register` 或 `/api/daemon/heartbeat` 的请求。守护进程机制详见 [守护进程与运行时](/daemon-runtimes)。

**可能原因**：

1. **`DIDIAN_SERVER_URL` 指错地址** —— 默认是 `ws://localhost:8080/ws`，self-host 要改成你自己的 server 地址
2. **网络 / 防火墙阻挡** —— daemon 和 server 不在同一网络，或出站被 block
3. **Token 过期或无效** —— 你从来没跑过 `didian login`，或 PAT 被撤销
...
```bash
didian daemon logs --lines 100    # 看 daemon 侧错误
echo $DIDIAN_SERVER_URL          # 确认地址配对
curl -i http://<server-host>:8080/health   # 直接戳 server
curl -i http://<server-host>:8080/readyz  # 连同 DB + migration readiness 一起检查
cat ~/.didian/config.json        # 看 api_token 是否存在
didian workspace list            # 确认你是目标工作区成员
```

**怎么修**：按上面原因对症处理。最常见的两个是**改 `DIDIAN_SERVER_URL` 重启 daemon**（`didian daemon restart`）和**重新登录**（`didian logout && didian login`）。

## 任务一直卡在 queued

**症状**：把 issue 分给 agent 后，issue 状态立刻变 `in_progress`，但过了很久页面没有 agent 执行的迹象；`didian daemon status` 显示 daemon `online`。
...
```bash
didian daemon status --output json       # runtime 列表 + last_seen_at
didian agent list                         # 查 agent 的 archived 状态
didian issue show <issue-id>             # 看 task 历史
```
...
- 并发打满 → 等现有任务跑完，或 `didian agent update <id> --max-concurrent-tasks 10` 提升上限
- 同 issue 串行 → 等前一个任务结束，或改分给不同 agent
- Agent 被 archive → `didian agent restore <id>`
- Runtime 未注册 → `didian daemon restart`，daemon 会重新注册
...
- Origin 错 → 在 server 的 `.env` 设 `FRONTEND_ORIGIN=https://didian.yourdomain.com`（或逗号分隔的 `CORS_ALLOWED_ORIGINS`），重启 server
...
| 错误日志 | 含义 | 怎么修 |
|---|---|---|
| `smtp dial <host>:<port>: dial tcp …: connect: connection refused` / `i/o timeout` | backend 容器连不上 relay —— host / port 错、防火墙挡了、或者 relay 没开 | 在容器里确认能解析和连通：`docker compose -f docker-compose.selfhost.yml exec backend nslookup <host>` 以及 `nc -vz <host> <port>`；放行从 Didian 主机到 relay 的网络 |
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | relay 的 receive connector 不允许这个子网把邮件中继到外部收件人（匿名内部 relay 发给外部域时常见） | 邀请仅限内部域，或者把 Didian 主机的子网加进 Exchange "Anonymous Users → Relay" 权限列表 |
...
- `Unable to relay` → 邀请仅限内部域，或在 Exchange receive connector 上给 Didian 主机授中继权限
...
1. **`DIDIAN_DEV_VERIFICATION_CODE` 为空** —— 固定验证码默认关闭
...
- 生产环境保持 `DIDIAN_DEV_VERIFICATION_CODE` 为空，配好 Resend 后使用真实验证码
- 本地开发或内网测试可以从 server 日志抄生成的验证码；如果需要 `888888`，设置 `APP_ENV=development` 和 `DIDIAN_DEV_VERIFICATION_CODE=888888`。不要在公网实例启用固定验证码（详见 [登录与注册配置 → 固定本地测试验证码](/auth-setup#固定本地测试验证码)）
...
2. **`pg_cron` 作为兼容路径配着、但指向了错的库** —— `pg_cron.database_name` 默认是 `postgres`；如果你的 Didian 数据库名不是 `postgres`，调度任务根本看不到 `rollup_task_usage_hourly()`。进程内调度器不依赖这一项，但如果你刻意拿掉了进程内调度而靠 `pg_cron`，DB 名就必须对得上。
...
**症状**：`didian server` 或 `didian daemon start` 启动失败，报 `address already in use`。
...
| 组件 | 位置 | 命令 |
|---|---|---|
| **守护进程** | `~/.didian/daemon.log`（后台模式）或前台 stdout | `didian daemon logs -f --lines 100` |
| **守护进程（崩溃）** | `~/.didian/daemon.err.log` | 直接打开——panic / 启动早期错误会写到这里 |
| **服务器（Docker）** | container stdout | `docker logs -f <container>` |
| **服务器（systemd）** | journal | `journalctl -u didian-server -f` |
| **前端（dev）** | `pnpm dev` 所在终端 | 直接看 |
| **前端（browser）** | DevTools → Console | 按 `F12` |

后台模式下 `daemon.log` 会按大小轮转（默认 20MB × 5 份 gzip 备份；可用 `DIDIAN_DAEMON_LOG_MAX_SIZE_MB` / `_MAX_BACKUPS` / `_MAX_AGE_DAYS` 调整），所以文件不会大到打不开。Go 运行时 panic 和 logger 建立前的错误会写到独立且有上限的 `daemon.err.log`。

需要更详细的 daemon 日志，把它从后台挪到前台跑：`didian daemon stop && didian daemon start --foreground`。前台运行会把日志实时打到终端，而不是写入轮转文件。
