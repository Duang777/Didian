...
This shape is intentional — it's the same pattern Didian already uses for agent providers: a `type` discriminator and a typed payload. It keeps the schema stable so adding "Notion page", "Google Doc", "uploaded file", or "external URL" later is a small, additive change.
...
`ref` is optional — if present, `didian repo checkout <url>` uses it as the default branch, tag, or commit for tasks in this project. An explicit `didian repo checkout <url> --ref <other-ref>` still wins for that one checkout.
...
The trade-off vs. `github_repo` is intentional: only the bound daemon can pick up tasks against the directory, and tasks on the same directory run **serially** instead of in parallel. In exchange you keep your existing checkout, your existing branch, your existing dirty state — Didian never re-clones it.
...
```bash
didian project resource add <project-id> \
  --type local_directory \
  --local-path /Users/me/code/big-game \
  --daemon-id <daemon-uuid> \
  --ref-label "main checkout"        # optional

didian project resource update <project-id> <resource-id> \
  --local-path /Users/me/code/big-game-new
```

`--daemon-id` comes from `didian daemon list`. The CLI also accepts the generic `--ref '<json>'` escape hatch if you'd rather pass the payload directly.
...
The blacklist is intentionally aggressive — picking your home directory would put Didian's runtime files at the root of your account, which is never what you want. Pick a sub-folder (typically your actual project checkout) instead.
...
3. Writes the agent's `CLAUDE.md` / `AGENTS.md` (and `.didian/project/resources.json`) **into the user's directory**. The agent works there, just as if you'd opened the folder yourself.
4. Keeps Didian's runtime artefacts (`output/`, `logs/`, `.gc_meta.json`) in a separate envRoot **outside** the user's directory.
...
### What Didian will and won't touch in your directory

- **Will write** `CLAUDE.md` / `AGENTS.md` (or the equivalent for your agent's provider) and `.didian/project/resources.json` at the directory root, so the agent has its meta-skill and resource list. Add these to your `.gitignore` if you don't want them committed.
...
didian project create \
  --title "Agent UX 2026" \
  --repo https://github.com/didian-ai/didian

# Manage resources later
didian project resource list <project-id>
didian project resource add  <project-id> --type github_repo --url <url>
didian project resource add  <project-id> --type github_repo --url <url> --ref <branch-or-sha>
didian project resource remove <project-id> <resource-id>
...
### 1. `.didian/project/resources.json`
...
Project resources (also written to `.didian/project/resources.json`):

- **GitHub repo**: https://github.com/didian-ai/didian (default branch: `main`)

Resources are pointers — open them only when relevant to the task. For
`github_repo` resources, use `didian repo checkout <url>` to fetch the code.
```
...
- **Caching / sync.** `github_repo` is just metadata — checkout still happens via `didian repo checkout` on demand. Cached document text for Notion / Google Docs will arrive with those types.
