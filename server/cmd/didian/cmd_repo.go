...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("DIDIAN_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("DIDIAN_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("DIDIAN_WORKSPACE_ID")
	agentName := os.Getenv("DIDIAN_AGENT_NAME")
	taskID := os.Getenv("DIDIAN_TASK_ID")
