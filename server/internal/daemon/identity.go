...
// same machine shares one UUID stored at `~/.didian/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.didian/profiles/*/daemon.id` and returns every
...
// explicitly pins DIDIAN_DAEMON_ID to the hostname itself, there's nothing
