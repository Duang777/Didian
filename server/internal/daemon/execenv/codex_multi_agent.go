...
// subagents. The Didian daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Didian runtime
...
//
// Users who explicitly want Codex native subagents inside a Didian task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `DIDIAN_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// DidianCodexMultiAgentEnv is the env var users can set to keep Codex
...
const DidianCodexMultiAgentEnv = "DIDIAN_CODEX_MULTI_AGENT"

// didianMultiAgentBeginMarker / didianMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	didianMultiAgentBeginMarker = "# BEGIN didian-managed multi-agent (do not edit; regenerated by daemon)"
	didianMultiAgentEndMarker   = "# END didian-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(didianMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(didianMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(DidianCodexMultiAgentEnv))
...
// renderDidianMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderDidianMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(didianMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(didianMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderDidianMultiAgentBlock(true)
...
//
// When DIDIAN_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per DIDIAN_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderDidianMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
