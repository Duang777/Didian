...
The Didian server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; Didian generates a unique URL of the shape:

```
https://<your-didian-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL — Didian records a run with `source = webhook`,
...
…or any JSON object/array. Didian normalizes it into an internal envelope:
...
When you don't provide an `event` field, Didian infers it from common
...
comma-separated **actions** list. Didian allows a webhook if **any**
...
Didian derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, Didian reads the
...
present, Didian falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, Didian
...
- **Rotate it if it leaks** — click "Rotate URL" on the trigger row, or run
  `didian autopilot trigger-rotate-url <autopilot-id> <trigger-id>`. The
...
Didian returns `200 OK` with a `status` field for normal no-op outcomes so
...
When `DIDIAN_PUBLIC_URL` is set on the server (e.g. `https://didian.example.com`),
...
custom self-hosted reverse proxies. Didian deliberately does not derive
