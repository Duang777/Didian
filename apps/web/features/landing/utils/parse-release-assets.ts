 ...
 *   didian-desktop-{version}-mac-{arch}.{dmg|zip}
 *   didian-desktop-{version}-windows-{arch}.exe
 *   didian-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^didian-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
