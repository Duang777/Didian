...
description: How to build the open-source Didian iOS app on your own iPhone — no App Store yet.
---

import { Callout } from "fumadocs-ui/components/callout";

Didian's iOS client is open-source and lives in the [main repo](https://github.com/didian-ai/didian) alongside web, desktop, and backend. It isn't on the App Store yet — until that changes, anyone who wants it on their iPhone builds from source. The build takes about 10–20 minutes the first time and ~2 minutes after that, and it talks to the same backend as [didian.ai](https://didian.ai) so your existing account just works.

<Callout type="info">
This page is for **personal use**. App developers should read [`apps/mobile/README.md`](https://github.com/didian-ai/didian/blob/main/apps/mobile/README.md) in the repo — it covers the dev / staging variants and the full script matrix.
</Callout>
...
- The Didian source code checked out:
...
There is no auto-update yet. When the Didian codebase moves forward, pull and rebuild:
...
The iOS app is still moving fast — the team prefers ship-and-iterate over App Store review cycles right now. A TestFlight beta is the most likely next step before a full App Store release. Until then, the self-build path above is the only way to use Didian on iOS.

If you'd like to be notified when TestFlight opens, watch the [GitHub repo](https://github.com/didian-ai/didian).

## Troubleshooting

**"No matching provisioning profiles found"** — Xcode refuses to sign the default bundle id `ai.didian.mobile` with your Apple ID. Rare, but happens if someone has registered that prefix on Apple's developer portal. Pick any reverse-domain you control (`com.yourname.didian` is fine), export it, and re-run:
...
**App can't reach the backend** — confirm `apps/mobile/.env.production` hasn't been modified (it ships with `EXPO_PUBLIC_API_URL=https://api.didian.ai`). If you changed it, restore with `git checkout apps/mobile/.env.production`.
