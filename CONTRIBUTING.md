...
This guide documents the local development workflow for contributors working on the Didian codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=didian`
...
POSTGRES_DB=didian
POSTGRES_PORT=5432
DATABASE_URL=postgres://didian:didian@localhost:5432/didian?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=didian_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://didian:didian@localhost:5432/didian_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../didian-feature -b feat/my-change main
cd ../didian-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `didian_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`didian login`).
...
whatever server is configured in `~/.didian/config.json`. That's fine for
...
Example: worktree at `../didian-feat-auth` produces profile
`dev-didian_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `DIDIAN_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.didian/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `didian` commands automatically use the same
...
rm -rf "$HOME/.didian/profiles/$PROFILE"
```
...
1. Compiles the `didian` CLI from `server/cmd/didian` into
   `apps/desktop/resources/bin/didian`
...
backend logs. If you set `DIDIAN_DEV_VERIFICATION_CODE=888888` before starting
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `Didian Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `Didian Canary`). Set either
...
Nothing in this flow touches the system-installed `didian` or the default
`~/.didian/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.didian/config.json` | `~/.didian/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.didian/daemon.pid` | `~/.didian/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/didian_workspaces/` | `~/didian_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `didian_<slug>_<hash>` |
| Desktop profile | `desktop-api.didian.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U didian -d postgres -At -c "select datname from pg_database order by datname;"
```
