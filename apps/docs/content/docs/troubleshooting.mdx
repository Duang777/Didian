...
description: Common issues when self-hosting Didian — symptoms, causes, how to diagnose, how to fix.
---

import { Callout } from "fumadocs-ui/components/callout";

Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/didian-ai/didian/issues).

## Daemon can't connect to the server

**Symptom**: [`didian daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).

**Likely causes**:

1. **`DIDIAN_SERVER_URL` points at the wrong address** — default is `ws://localhost:8080/ws`; self-host must change it to your server address
2. **Network / firewall blocking** — the daemon and server aren't on the same network, or outbound traffic is blocked
3. **Token expired or invalid** — you never ran `didian login`, or the PAT was revoked
...
```bash
didian daemon logs --lines 100    # look for daemon-side errors
echo $DIDIAN_SERVER_URL          # confirm the address is set
...
cat ~/.didian/config.json        # verify api_token exists
didian workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `DIDIAN_SERVER_URL` and restarting the daemon** (`didian daemon restart`) and **signing in again** (`didian logout && didian login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `didian daemon status` shows the daemon `online`.
...
```bash
didian daemon status --output json       # runtime list + last_seen_at
didian agent list                         # check agent archived state
didian issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `didian agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `didian agent restore <id>`
- Runtime not registered → `didian daemon restart`, and the daemon will re-register
...
- Wrong origin → set `FRONTEND_ORIGIN=https://didian.yourdomain.com` in the server's `.env` (or comma-separated `CORS_ALLOWED_ORIGINS`) and restart the server
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the Didian host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the Didian host's IP relay permission on the Exchange receive connector
...
1. **`DIDIAN_DEV_VERIFICATION_CODE` is empty** — fixed codes are disabled by default
...
```bash
cat .env | grep -E 'APP_ENV|DIDIAN_DEV_VERIFICATION_CODE'
docker exec <container> env | grep -E 'APP_ENV|DIDIAN_DEV_VERIFICATION_CODE'
```
...
**Symptom**: `didian server` or `didian daemon start` fails with `address already in use`.
...
|---|---|---|
| **Daemon** | `~/.didian/daemon.log` (background mode) or foreground stdout | `didian daemon logs -f --lines 100` |
| **Daemon (crashes)** | `~/.didian/daemon.err.log` | Open directly — panics / early-startup errors land here |
| **Server (Docker)** | Container stdout | `docker logs -f <container>` |
| **Server (systemd)** | journal | `journalctl -u didian-server -f` |
| **Frontend (dev)** | Terminal running `pnpm dev` | Read directly |
| **Frontend (browser)** | DevTools → Console | Press `F12` |

In background mode `daemon.log` is size-rotated (20MB × 5 gzip backups by default; tune with `DIDIAN_DAEMON_LOG_MAX_SIZE_MB` / `_MAX_BACKUPS` / `_MAX_AGE_DAYS`), so it stays small enough to open. Go runtime panics and pre-logger errors go to the separate, bounded `daemon.err.log`.

For more detailed daemon logs, move it from background to foreground: `didian daemon stop && didian daemon start --foreground`. Running in the foreground streams logs live to your terminal instead of the rotated file.
