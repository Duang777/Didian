...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    ios: {
      ...
      // Didian Apple Developer team (and external users self-building a
      ...
      bundleIdentifier: isProd
        ? (process.env.EXPO_BUNDLE_IDENTIFIER_PROD ?? "ai.didian.mobile")
        : isStaging
          ? "ai.didian.mobile.staging"
          : (process.env.EXPO_BUNDLE_IDENTIFIER_DEV ?? "ai.didian.mobile.dev"),
    },
    plugins: [
      "expo-router",
      "expo-secure-store",
