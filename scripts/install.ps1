# Didian installer for Windows — one command to get started.
#
# Install CLI (default): connects to didian.ai
#   irm https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Didian server + installs CLI + configures
#   $env:DIDIAN_MODE="local"; irm https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/didian-ai/didian.git"
$RepoWebUrl    = "https://github.com/didian-ai/didian"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".didian\server"
$InstallDir    = if ($env:DIDIAN_INSTALL_DIR) { $env:DIDIAN_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/didian-ai/didian/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:DIDIAN_SELFHOST_REF) {
        return $env:DIDIAN_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = didian version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Didian CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Didian requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/didian-ai/didian/releases/download/$latest/didian-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "didian-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "didian.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/didian-ai/didian/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "didian.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "didian-cli-$version-windows-$arch.zip"
        $legacyAsset = "didian_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "didian.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".didian\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "didian.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "didian.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "didian.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "didian.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Didian CLI installed to $binDir\didian.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "didian") {
        ...
        if ($isUpToDate) {
            Write-Ok "Didian CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Didian CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Didian CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "didian")) {
        Write-Fail "CLI installed but 'didian' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Didian self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:DIDIAN_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Didian server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Didian repository..."
    ...
    Write-Info "Pulling official Didian images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Didian services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Didian server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Didian - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Didian CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     didian setup               " -NoNewline; Write-Host "# Connect to Didian Cloud (didian.ai)" -ForegroundColor DarkGray
    Write-Host "     didian setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:DIDIAN_MODE="with-server"; irm https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Didian - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Didian server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     didian setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:DIDIAN_MODE="stop"; irm https://raw.githubusercontent.com/didian-ai/didian/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Didian services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Didian installation found at $InstallDir"
    }

    if (Test-CommandExists "didian") {
        try {
            didian daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:DIDIAN_MODE) { $env:DIDIAN_MODE.ToLower() } else { "default" }
