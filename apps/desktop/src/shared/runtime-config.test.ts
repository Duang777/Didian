...
describe("runtime config", () => {
  it("uses cloud defaults without a desktop.json file", () => {
    expect(DEFAULT_RUNTIME_CONFIG).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.didian.ai",
      wsUrl: "wss://api.didian.ai/ws",
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.didian.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.didian.ai",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/didian" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. Didian's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.didian.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.didian.ai",
      wsUrl: "wss://api.test.didian.ai/ws",
      appUrl: "https://test.didian.ai",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.didian.ai",
        appUrl: "https://staging.didian.ai",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.didian.ai",
