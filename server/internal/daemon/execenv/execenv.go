...
// checked out on demand by the agent via `didian repo checkout`.
package execenv
...
// on demand via `didian repo checkout <url>`.
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	// that lose all DIDIAN_* env vars AND escape above the workdir. Non-fatal:
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// a collision-free sibling (issue-review, issue-review-didian,
	// issue-review-didian-2, …), accumulating a fresh duplicate on every
	...
	//      refresh back to issue-review-didian.
