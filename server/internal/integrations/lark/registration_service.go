...
func (s *RegistrationService) BeginInstall(ctx context.Context, p BeginInstallParams) (BeginInstallResult, error) {
	...
	// bot reads "<agent> - Didian" instead of "{用户姓名}的智能助手".
...
// "connected to a different Didian workspace" that lied when the real owner sat
...
func (s *RegistrationService) liveOwnerConflictMessage(ctx context.Context, requestingWorkspaceID pgtype.UUID, appID string) string {
	...
	case owner.WorkspaceID != requestingWorkspaceID:
		return "This Feishu app is already connected to a different Didian workspace. Disconnect it there before connecting it here."
...
// "<agent> - Didian" instead of Lark's auto-generated
...
// "Didian" rather than a dangling " - Didian".
