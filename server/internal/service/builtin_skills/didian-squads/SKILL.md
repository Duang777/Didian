---
name: didian-squads
description: "Use when creating, inspecting, updating, assigning, mentioning, or debugging Didian squads. Explains what squads are, squad/member fields, CLI commands, leader routing, issue assignment, comments, mentions, autopilot behavior, leader briefing, side effects, and product-gap handling."
...
didian squad get <squad-id> --output json
didian squad member list <squad-id> --output json
didian issue comment list <issue-id> --recent 10 --output json
```
...
```bash
didian squad --help
didian squad member --help
didian issue update --help
...
A Didian squad is a workspace routing and coordination object.
...
```bash
didian squad list --output json
didian squad get <squad-id> --output json
didian squad create --name <name> --leader <agent-name-or-id> --output json
didian squad update <squad-id> --instructions "<leader coordination policy>" --output json
didian squad delete <squad-id>
```
...
```bash
didian squad member list <squad-id> --output json
didian squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
didian squad member remove <squad-id> --member-id <id> --type agent|member
didian squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
didian squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
didian issue get <issue-id> --output json
didian issue update <issue-id> --help
didian issue comment list <issue-id> --output json
didian issue comment add <issue-id> --help
```
...
For squad leader tasks, Didian appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `didian-*` skills are not listed —
...
- recording squad activity with `didian squad activity`;
