...
func NewRealtimeCollector(m *realtime.Metrics) *RealtimeCollector {
	return &RealtimeCollector{
		...
		redisMirrorErrors:   prometheus.NewDesc("didian_realtime_redis_mirror_errors_total", "Total Redis mirror write errors by the realtime relay.", []string{"target"}, nil),
...
func newRealtimeDesc(name, help string) *prometheus.Desc {
	return prometheus.NewDesc("didian_realtime_"+name, help, nil, nil)
}
