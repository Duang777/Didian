...
func BuildPrompt(task Task, provider string) string {
	...
	b.WriteString("You are running as a local coding agent for a Didian workspace.\n\n")
...
// the agent's job is to translate it into one `didian issue create` CLI
...
// call `didian issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Didian workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `didian issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `didian workspace member list --output json`, `didian agent list --output json`, and `didian squad list --output json` and find the matching entity by display name. Squads are first-class assignees too — a squad name (e.g. \"Super Human\") routes work to the squad leader, who then delegates. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent or squad) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	...
	b.WriteString("- Run exactly one `didian issue create --output json` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `didian issue get` or `didian issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if len(task.CoalescedComments) > 0 {
			...
			fmt.Fprintf(&b, "\nIf you need the surrounding discussion for any of them, fetch its thread with `didian issue comment list %s --thread <thread-id> --tail 30 --output json` using the thread id shown above.\n\n", task.IssueID)
		} else if len(task.CoalescedCommentIDs) > 0 {
			fmt.Fprintf(&b, "This run also covers %d earlier comment(s) posted before it started — you must read and address them too, not just the one above: %s. These may be in DIFFERENT threads, so do not assume they share the triggering thread; fetch each by pulling the issue-wide discussion with `didian issue comment list %s --recent 30 --output json` (expand with `--full` if a thread is folded) and locate the ids above.\n\n",
		...
		if task.Agent != nil && strings.Contains(task.Agent.Instructions, "## Squad Operating Protocol") {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `didian squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise.\n\n", task.IssueID)
		}
	}
	fmt.Fprintf(&b, "Start by running `didian issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	...
	if hint := execenv.BuildNewCommentsHint(task.IssueID, task.TriggerCommentID, task.TriggerThreadID, task.NewCommentsSince, task.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(&b, "Read the discussion: `didian issue comment list %s --recent 10 --output json` (resolved threads come back folded — `--full` to expand).\n\n", task.IssueID)
	}
...
func buildChatPrompt(task Task) string {
	...
	if task.ChatIntro {
		var b strings.Builder
		b.WriteString("You are running as a chat assistant for a Didian workspace.\n")
	...
	// like "what did you just talk about" sends it to read Didian instead of the
	// Slack conversation. State it explicitly, point reads at the channel (not
	// Didian), and teach the two read commands, telling the agent which to start
	// with based on where it was @mentioned. A web-only chat session gets no such
	// block — its history is the Didian chat_session the agent already resumes.
	if task.ChatChannelType != "" {
		platform := channelDisplayName(task.ChatChannelType)
		fmt.Fprintf(&b, "You are operating inside a %s conversation — not the Didian web app. This conversation and its history live in %s, NOT in Didian; never look in Didian issues or comments for it. The message below may be only what triggered you. Read the conversation with:\n", platform, platform)
		b.WriteString("- `didian chat history --output json` — the channel overview: recent top-level messages, each thread tagged with a `thread_id` and `reply_count`. It does NOT expand thread contents.\n")
		b.WriteString("- `didian chat thread [<thread_id>] --output json` — read one thread's messages; omit the id to read the thread you are in, or pass a `thread_id` from the overview to read a specific thread.\n")
		if task.ChatInThread {
			b.WriteString("You were @mentioned inside a thread: start with `didian chat thread` to read it; if you need the wider channel, run `didian chat history` and open a specific thread with `didian chat thread <thread_id>`.\n")
		} else {
			b.WriteString("You were @mentioned at the channel top level: start with `didian chat history` to see the channel, then read a specific thread's contents with `didian chat thread <thread_id>`.\n")
		}
	...
	// have expired. `didian attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `didian attachment download <id>` to fetch each file locally before referring to it.\n")
		b.WriteString("When creating an issue that should preserve one of these attachments, pass `--attachment-id <id>` to `didian issue create` in addition to keeping the attachment markdown inline.\n")
	}
	return b.String()
}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `didian autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
		b.WriteString("Complete the instructions above.\n")
	}
	b.WriteString("Do not run `didian issue get`; this run does not have an issue ID.\n")
	return b.String()
}
