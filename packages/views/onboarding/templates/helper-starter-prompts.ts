 ...
 * Didian Helper is created. Each card maps to ONE issue being created
...
export const HELPER_STARTER_PROMPTS: Record<StarterCardId, StarterPrompt> = {
  intro: {
    title: {
      en: "Introduce Didian to me",
      zh: "简单介绍一下 Didian",
      ko: "Didian를 간단히 소개해 주세요",
      ja: "Didian を簡単に紹介してください",
    },
    prompt: {
      en: "Introduce Didian to me in 1–2 paragraphs. Cover what it is, the core concepts (workspace / issue / agent / runtime), and how it differs from tools like Linear or Jira.",
      zh: "用 1-2 段话简单介绍 Didian 给我。讲清楚它是什么、核心概念有哪些(workspace / issue / agent / runtime)、和 Linear / Jira 之类的工具核心区别在哪。",
      ko: "Didian를 1-2문단으로 간단히 소개해 주세요. 무엇인지, 핵심 개념(workspace / issue / agent / runtime)이 무엇인지, Linear나 Jira 같은 도구와 핵심적으로 어떻게 다른지 설명해 주세요.",
      ja: "Didian を1〜2段落で簡単に紹介してください。何であるか、中心となる概念(workspace / issue / agent / runtime)、そして Linear や Jira のようなツールと根本的にどう違うのかを説明してください。",
    },
  },
  tour: {
    title: {
    ...
    prompt: {
      en: "Walk me through Didian's core features — issue, agent, squad, autopilot, chat. Pick one realistic scenario I might run into and explain how all these pieces fit together.",
      zh: "陪我熟悉 Didian 的每个核心功能 —— issue、agent、squad、autopilot、chat。挑一个我可能用得上的真实场景,讲讲这几个东西是怎么配合的。",
      ko: "Didian의 핵심 기능인 issue, agent, squad, autopilot, chat을 안내해 주세요. 제가 실제로 겪을 만한 상황 하나를 골라 이 요소들이 어떻게 함께 작동하는지 설명해 주세요.",
      ja: "Didian の主要な機能 — issue、agent、squad、autopilot、chat を案内してください。私が実際に遭遇しそうな現実的なシナリオを1つ選び、これらの要素がどう連携するのかを説明してください。",
    },
  },
  welcome_page: {
    title: {
      en: "Show me what Didian can do for me — as slides",
      zh: "用 slides 介绍 Didian 能为我做什么",
      ko: "Didian가 저에게 무엇을 해줄 수 있는지 슬라이드로 보여 주세요",
...
- 5–8 slides total:
  1. Title — "What Didian can do for [my role]"
  2. Four core concepts — workspace / issue / agent / runtime, one slide
  3–6. 3–4 concrete scenarios tailored to my use case, each in the form "When you want X → here's how Didian handles it"
...
- 5-8 张 slide:
  1. 标题页 —— "Didian 能为 [我的角色] 做什么"
  2. 四个核心概念 —— workspace / issue / agent / runtime,一张
  3-6. 3-4 个针对我使用场景的具体例子,形如"当你想做 X → Didian 是这样处理的"
  7. 收尾页 —— 一个具体的下一步动作
...
- 전체 5-8장:
  1. 제목 — "[내 역할]에게 Didian가 해줄 수 있는 일"
  2. 네 가지 핵심 개념 — workspace / issue / agent / runtime, 한 장
  3-6. 제 사용 사례에 맞춘 구체적인 시나리오 3-4개. 각 시나리오는 "X를 하고 싶을 때 → Didian는 이렇게 처리합니다" 형식
  7. 마무리 — 구체적인 다음 액션 하나
...
- 全体で5〜8枚のスライド:
  1. タイトル — 「[私の役割] に Didian ができること」
  2. 4つの中心概念 — workspace / issue / agent / runtime を1枚で
  3〜6. 私のユースケースに合わせた具体的なシナリオを3〜4個。それぞれ「X をしたいとき → Didian はこう処理します」の形で
  7. 締め — 具体的な次の一歩のアクション
