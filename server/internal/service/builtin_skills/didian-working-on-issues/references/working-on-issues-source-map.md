...
## `didian issue pull-requests` — read PR links from Didian

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/didian/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/didian/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/didian/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `didian issue pull-requests` later reads back.
...
| `--stage` on `issue create` / `issue update` | `server/cmd/didian/cmd_issue.go:328,350` |
| `didian issue children <id>` (sub-issues grouped by stage) | `server/cmd/didian/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
| Behavior | File:line |
|---|---|
| `didian issue metadata set <issue-id> --key --value [--type]` | `server/cmd/didian/cmd_issue_metadata.go:80,109-111` |
| `didian issue metadata delete <issue-id> --key` | `server/cmd/didian/cmd_issue_metadata.go:93,113` |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/didian/cmd_issue.go
