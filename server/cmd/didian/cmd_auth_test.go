...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers DIDIAN_APP_URL", func(t *testing.T) {
		t.Setenv("DIDIAN_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://didian.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `didian login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
// `didian login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Didian Cloud Node PATs (`mcn_`) at
