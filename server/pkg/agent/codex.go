...
const (
	didianCodexMcpBeginMarker = "# BEGIN didian-managed mcp_servers (do not edit; regenerated by daemon)"
	didianCodexMcpEndMarker   = "# END didian-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(didianCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(didianCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = didianCodexMcpBeginMarker + "\n" + didianCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(didianCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(didianCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isDidianMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "didian-agent-sdk",
				"title":   "Didian Agent SDK",
				"version": "0.2.0",
			},
