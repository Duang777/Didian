---
name: didian-working-on-issues
description: "Use when working on a Didian issue after the runtime has provided the trigger context — to apply the product contracts the runtime brief does not encode: how PR linking differs from close intent, how to read a linked PR's real state via the pull-requests CLI, which metadata keys are high-signal, what status changes trigger on the server, and how sub-issue create status (todo vs backlog) controls whether assigned agents start immediately."
user-invocable: false
allowed-tools: Bash(didian *), Bash(git *), Bash(gh *)
---

# Working on Didian issues
...
For building mention links, load `didian-mentioning` instead — not this skill.
...
This is the link that `didian issue pull-requests` reads back — but see the
...
**excluded from `didian issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Didian issue comment, unless
...
When a step depends on PR state, query Didian's link table — do not infer it
from branch names, GitHub search, memory, or `pr_url` metadata (which can be
stale).

```bash
didian issue pull-requests <issue-id> --output json
```
...
```bash
didian issue metadata set <issue-id> --key pr_url --value <url>
didian issue metadata delete <issue-id> --key <stale-key>
```
...
```bash
didian issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
didian issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
didian issue status <child-id> todo   # promote when the previous step is truly done
```
...
didian issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
didian issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
didian issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
didian issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
didian issue children <parent-id>             # sub-issues grouped by stage
didian issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
didian issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
didian issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
didian issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
didian issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
didian issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
