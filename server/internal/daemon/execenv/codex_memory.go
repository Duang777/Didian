...
// — users cannot audit or edit the contents from any Didian UI.
//
// This conflicts with the Didian daemon's context model. Didian
...
//     unrelated host-project context into Didian tasks. The reproduction
//     in github.com/didian-ai/didian#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Didian issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Didian task
// (and accept the leak risk) can keep the feature enabled by setting
// `DIDIAN_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Didian-owned, user-visible,
...
// DidianCodexMemoryEnv is the env var users can set to keep Codex
...
const DidianCodexMemoryEnv = "DIDIAN_CODEX_MEMORY"

const (
	didianMemoryFeatureBeginMarker = "# BEGIN didian-managed memory-feature (do not edit; regenerated by daemon)"
	didianMemoryFeatureEndMarker   = "# END didian-managed memory-feature"
	didianMemoryConfigBeginMarker  = "# BEGIN didian-managed memory-config (do not edit; regenerated by daemon)"
	didianMemoryConfigEndMarker    = "# END didian-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(didianMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(didianMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(didianMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(didianMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(DidianCodexMemoryEnv))
...
// renderDidianMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderDidianMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(didianMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(didianMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderDidianMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderDidianMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(didianMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(didianMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderDidianMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderDidianMemoryConfigBlock(true))
}
...
//
// When DIDIAN_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per DIDIAN_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderDidianMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderDidianMemoryConfigBlock(false))
	}
