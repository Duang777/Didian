---
name: didian-runtimes-and-repos
description: "Use when inspecting or debugging Didian runtimes, daemon task claiming, agent not running, workdir/session reuse, or repository checkout. Covers runtime online/offline state, daemon heartbeat/claim chain, task-scoped repo checkout, project repo context, local_directory caveats, and safe diagnostic commands."
user-invocable: false
allowed-tools: Bash(didian *)
---

# Didian Runtimes and Repos
...
7. `didian repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
didian runtime list --output json
didian runtime usage <runtime-id> --output json
didian runtime activity <runtime-id> --output json
didian runtime update <runtime-id> --target-version <version> --output json
didian runtime delete <runtime-id>
didian repo checkout <url>
didian repo checkout <url> --ref <branch-or-sha>
```
...
4. Is the runtime online? `didian runtime list --output json`.
