...
- `import type` from `@didian/core/types/*` (zero runtime coupling)
- Pure functions from `@didian/core/`
...
- **Main CI** (`.github/workflows/ci.yml`) excludes mobile via `--filter='!@didian/mobile'`. Mobile failures do NOT block web/desktop PRs.
...
1. **Read the payload.** Find the event in `@didian/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
