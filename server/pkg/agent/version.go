...
// the didian CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `didian issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest didian CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("didian CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("didian CLI version is below required minimum")
)
