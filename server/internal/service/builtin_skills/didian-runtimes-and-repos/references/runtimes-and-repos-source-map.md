...
- `server/cmd/didian/cmd_runtime.go` registers `runtime list`, `usage`, `activity`, `update`, and `delete`.
...
- `server/cmd/didian/cmd_repo.go` registers `repo checkout <url> [--ref]`.
- `repo checkout` requires `DIDIAN_DAEMON_PORT`, sends `workspace_id`, `workdir`, `ref`, `agent_name`, and `task_id` to local daemon `/repo/checkout`, then prints the checked-out path.
