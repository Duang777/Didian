...
// `no such host` errors when calling out (for example, `didian issue get`
// hitting the Didian API). See upstream issue openai/codex#10390.
//
...
// actually reach the Didian API. On Linux (and on macOS once the upstream
...
//     danger-full-access so the Didian CLI can reach the API.
...
// didianManagedBeginMarker / didianManagedEndMarker delimit the block the
...
const (
	didianManagedBeginMarker = "# BEGIN didian-managed (do not edit; regenerated by daemon)"
	didianManagedEndMarker   = "# END didian-managed"
)

// renderDidianManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.didian]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertDidianManagedBlock), avoids both traps.
func renderDidianManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(didianManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(didianManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(didianManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(didianManagedEndMarker) + `\n*`)

// upsertDidianManagedBlock returns the config content with the didian-managed
...
// `[permissions.didian]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderDidianManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertDidianManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderDidianManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the didian-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertDidianManagedBlock(existing, policy)
