...
// nothing related to Didian before a task ran must contain nothing
// related to Didian after the task finishes — no .agent_context/,
// no .claude/skills/, no .didian/, no stub directories. The test
...
// must still exist and the Didian-written sibling must be gone.
...
// round-trip to the .didian/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .didian/sidecar.txt and created
	// .didian/ + .didian/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .didian/.
	managedDir := filepath.Join(workDir, ".didian")
	...
	// .didian still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-didian/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Didian would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Didian skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Didian
// sibling (which lives at `<slug>-didian`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Didian's version",
						Content:     "---\nname: issue-review\n---\n\nDidian skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Didian-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.didian/project/
// resources.json` — the other Didian-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".didian", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".didian", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-didian`, then `-didian-2`,
// `-didian-3`, … The PR-review concern is "Didian skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-didian`.
	...
	if slug != "issue-review-didian" {
		t.Errorf("second allocation should bump to `-didian`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-didian") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-didian"), 0o755); err != nil {
	...
	if slug != "issue-review-didian-2" {
		t.Errorf("third allocation should be `-didian-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-didian-2") {
...
// `Issue Review` skill, the Didian sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Didian sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Didian skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Didian's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	didianDir := filepath.Join(workDir, ".claude", "skills", "issue-review-didian")
	if _, err := os.Stat(filepath.Join(didianDir, "SKILL.md")); err != nil {
		t.Errorf("Didian sibling skill should exist at %s: %v", didianDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Didian sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(didianDir); !os.IsNotExist(err) {
		t.Errorf("Didian sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
