...
description: Agents don't run on Didian's servers — they run on your own machines.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

In Didian, [agents](/agents) do **not** run on our servers — they run on your own machines, driven by a small program called the **daemon** that invokes the [AI coding tools](/providers) installed locally. The Didian server only coordinates: it stores [issues](/issues), queues [tasks](/tasks), and dispatches them to the right **runtime** (runtime = daemon × one AI coding tool).

This structure is the biggest difference between Didian and Linear / Jira: **your API keys, toolchain, and code directories stay on your machine** — the Didian server never sees any of them. That means "my agent isn't working" is almost always a local problem — the daemon isn't running, an AI tool isn't installed, a key has expired. Check locally first; see [Troubleshooting](/troubleshooting) for a guide.

## Starting the daemon

The daemon is part of the Didian CLI. Once you've installed the [Didian CLI](/cli), run on your own machine:
...
| Command | Purpose |
|---|---|
| `didian daemon start` | Start (background by default; add `--foreground` to run in the foreground) |
| `didian daemon stop` | Stop |
| `didian daemon restart` | Restart |
| `didian daemon status` | Show status |
| `didian daemon logs` | Show logs (add `-f` to follow) |

Full CLI reference in [CLI commands](/cli).

**The desktop app ships with a daemon.** If you use the [desktop app](/desktop-app), you don't need to run `didian daemon start` manually — it launches the daemon automatically on startup. See the [Desktop app](/desktop-app) page for which option fits your workflow.

## Why one machine has multiple runtimes

A runtime is not a server and not a container — it's the combination of "**daemon × one AI coding tool**". For example: you start the daemon on a MacBook with both Claude Code and Codex installed, and you're a member of two workspaces. Didian then registers 4 runtimes:
...
- The **Runtimes** page in the Didian UI lists these rows

## Custom runtime profiles

Built-in provider detection covers the common tools, but teams can also define
**custom runtime profiles** for AI CLIs that speak one of Didian's supported
protocol families and need a workspace-specific launch command. Profiles are
managed from the Runtimes UI or the CLI:

```bash
didian runtime profile list
didian runtime profile create --display-name "Composer" --protocol-family codex --command-name agent
didian runtime profile update <profile-id> --command-name agent
didian runtime profile delete <profile-id>
```

The command is argv-oriented, not a shell string. Didian stores an executable
...
didian runtime profile unset-path <profile-id>
```
...
<Callout type="info">
**Cloud runtimes are coming**, currently in a waitlist phase. Once available, you'll be able to execute agent tasks directly on Didian Cloud without running a local daemon. Sign up with your email on the [download page](https://didian.ai/download) to get notified.
</Callout>

## When a runtime is marked offline

Didian uses heartbeats to decide whether a runtime is online. Three key numbers:
...
**Tasks running on a missing runtime are marked as failed** (failure reason `runtime_offline`). For retryable sources (issues, chat), Didian automatically requeues them; Autopilot-triggered tasks are not retried automatically. See [Tasks → Which failures retry automatically](/tasks#which-failures-retry-automatically-which-dont).
</Callout>

## How many tasks can run in parallel

Didian enforces concurrency limits at two layers:

- **Daemon layer**: **20 concurrent tasks** by default (tunable via env var `DIDIAN_DAEMON_MAX_CONCURRENT_TASKS`)
...
1. Run `didian daemon status` — confirm the daemon is running and online
2. Run `didian daemon logs -f` — check for errors
3. Open the **Runtimes** page in the Didian UI — confirm your runtime shows "online"
