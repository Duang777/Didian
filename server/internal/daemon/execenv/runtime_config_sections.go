...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Didian Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Didian platform. Use the `didian` CLI to interact with the platform.\n\n")
}
...
func writeBackgroundTaskSafetySlim(b *strings.Builder) {
	...
	b.WriteString("Didian marks the task terminal the moment your top-level turn exits — any background work still running is orphaned, its result lost, and the final comment you meant to post after it never sends. There is no background-completion wakeup here.\n\n")
	...
	b.WriteString("- If a tool response says to wait for a future notification/reminder, or that it is running in the background so you can keep working, do not rely on that in Didian-managed runs — block on the appropriate wait / output / collect operation before exiting.\n")
...
func writeTaskInitiator(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Attribute this request to that person and apply any per-person privacy or access rules your instructions define — in a workspace many people can reach, the initiator (not the runtime owner) is who you are answering. Your Didian credentials stay scoped to the runtime owner, so this attribution does not widen what you can read or write — do not assume the initiator can see everything you can.\n\n")
}
...
// preserved: each `didian issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `didian issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `didian --help` or `didian <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `didian issue get <id> --output json` — full issue.\n")
	b.WriteString("- `didian issue comment list <issue-id> [--thread <comment-id> [--tail N] | --recent N] [--before <ts> --before-id <uuid>] [--since <RFC3339>] [--full] --output json` — thread-aware comment reads. Resolved threads come back folded by default on complete-thread reads (default list, `--recent`, `--thread` without `--tail`); pass `--full` to expand. Page older replies / threads with `--before`/`--before-id` (stderr labels: `Next reply cursor`, `Next thread cursor`); `--help` for full semantics.\n")
	...
	b.WriteString("- `didian issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <RFC3339>]` — update fields; pass `--parent \"\"` to clear parent.\n")
	...
	b.WriteString("- `didian repo checkout <url> [--ref <branch-or-sha>]` — git worktree on a dedicated branch.\n\n")
	b.WriteString("### Squad maintenance\n")
	b.WriteString("- `didian squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
}
...
// every CLI other than `didian issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `didian --help` or `didian <command> --help`.\n\n")
	b.WriteString("### Core\n")
...
func writeCommentFormatting(b *strings.Builder) {
	...
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin` (PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to a native command, silently dropping non-ASCII characters as `?` before they reach `didian.exe`). Never use inline `--content` for agent-authored comments. Write that file inside your working directory (`./reply.md`), never `/tmp` or shared paths — the CLI rejects a `--content-file` path outside the workdir so another run's stale file can't leak in (MUL-4252). Keep the same `--parent` value from the trigger comment when replying. Delete the temp file (`Remove-Item ./reply.md`) after posting; do not rely on `\\n` escapes.\n\n")
		return
	}
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `didian repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a git worktree on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.didian/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `didian repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Pin only if BOTH: (a) materially important to this issue, AND (b) a future run is likely to re-read it. Otherwise leave the bag alone. Stale keys: overwrite with the new value or `didian issue metadata delete`.\n")
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `didian` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `didian issue list --output json` or `didian issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `didian workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `didian repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Didian issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Didian issue for this run.\n\n")
	...
	if ctx.AutopilotID != "" {
		fmt.Fprintf(b, "- Run `didian autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
	}
	b.WriteString("- Complete the autopilot instructions directly\n")
	b.WriteString("- Do not run `didian issue get`, `didian issue comment add`, or `didian issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue\n\n")
}
...
func writeWorkflowComment(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	fmt.Fprintf(b, "1. Run `didian issue get %s --output json` to understand the issue context\n", ctx.IssueID)
	fmt.Fprintf(b, "2. Run `didian issue metadata list %s --output json` to see what prior agents pinned — best-effort, empty `{}` and CLI failures are normal. See the `## Issue Metadata` section above for what to look for.\n", ctx.IssueID)
	if hint := BuildNewCommentsHint(ctx.IssueID, ctx.TriggerCommentID, ctx.TriggerThreadID, ctx.NewCommentsSince, ctx.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(b, "3. Catch up on comments — read with `didian issue comment list %s --recent 10 --output json` (resolved threads come back folded — `--full` to expand).\n", ctx.IssueID)
	}
	...
	b.WriteString("8. Before exiting: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `didian issue metadata set`/`delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n")
...
func writeWorkflowAssignment(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if ctx.IsSquadLeader {
		fmt.Fprintf(b, "6. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `didian squad activity %s no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `didian issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID, ctx.IssueID)
	} else {
		fmt.Fprintf(b, "6. **Post your final results as a comment — this step is mandatory**: post it with `didian issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID)
	}
	b.WriteString("7. Before exiting: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `didian issue metadata set`/`delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n")
	fmt.Fprintf(b, "8. When done, run `didian issue status %s in_review` unless your Agent Identity forbids issue status changes; if it does, skip this step.\n", ctx.IssueID)
	fmt.Fprintf(b, "9. If blocked, run `didian issue status %s blocked` unless your Agent Identity forbids issue status changes. Post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n", ctx.IssueID)
}
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("**Choosing `--status` when creating sub-issues.** `--status todo` = **start now** (default — agent assignees fire immediately). `--status backlog` = **wait**, then promote later with `didian issue status <child-id> todo`. Parallel children: all `--status todo`. Strict serial 1→2→3: only Step 1 `todo`, Steps 2/3 `--status backlog` from the start.\n\n")
	b.WriteString("**Ordering with stages.** For phased plans, group children with `--stage <N>` (N ≥ 1) instead of hand-promoting the backlog chain — stage members run together, and the parent wakes once per stage. Use `--stage k --status backlog` for later stages, then `didian issue children <id>` to inspect groupings before promoting. Reach for stages whenever a plan has more than one step or a step must wait for a group.\n\n")
}
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("When a task includes attachment IDs and you need the files, inspect `didian attachment --help` and use the authenticated CLI path. Do not open Didian resource URLs directly.\n\n")
}

// writeAlwaysUseCLI emits the "must go through the didian CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `didian` CLI\n\n")
	b.WriteString("Access Didian platform resources (issues, comments, attachments, files) only through the `didian` CLI — never `curl` / `wget`. For any operation the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `didian issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `didian issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `didian issue create`. Use the created issue's `identifier` from JSON output when available; otherwise use its `id`. Do not assume any workspace issue prefix such as `MUL-`; workspaces can use custom prefixes.\n")
	...
	default:
		if ctx.IsSquadLeader {
			...
			b.WriteString("⚠️ **Final results MUST be delivered via `didian issue comment add`.** The user does NOT see your terminal output, assistant chat text, or run logs — only comments on the issue. A task that finishes without a result comment is invisible to the user, even if the work itself was correct.\n\n")
		}
