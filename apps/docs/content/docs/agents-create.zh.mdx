...
运行时背后是一款具体的 AI 编程工具。Didian 支持 14 款，最常用的几款：
...
系统关键变量不能被覆盖：`PATH`、`HOME`、`USER`、`SHELL`、`TERM`、`CODEX_HOME`，以及任何 `DIDIAN_*` 开头的 key 都会被守护进程静默忽略（日志里有 warn，不会报错）。

<Callout type="warning">
**`custom_env` 的值在 Didian 服务器的数据库里是明文存储的。** agent list/get 接口不再返回任何 env 值——只返回键的数量。读取真实值需要 workspace owner / admin 调用专用且会审计的 `GET /api/agents/{id}/env`（CLI: `didian agent env get <id>`）。正在执行任务的智能体即便宿主是 owner，也无法借此读取其它智能体的 env——这个接口拒绝 agent-actor 会话。数据库备份、DB 审计里仍然能看到真实值。
