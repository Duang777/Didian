...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Didian", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/didian") {
		t.Errorf("qr source=%q want go-sdk/didian", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Didian" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Didian" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Didian")
	}
}
