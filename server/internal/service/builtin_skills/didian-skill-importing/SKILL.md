---
name: didian-skill-importing
description: "Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Didian workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, --on-conflict fail|overwrite|rename|skip behavior and structured import results, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Didian install."
user-invocable: false
allowed-tools: Bash(didian *)
---

# Importing skills into Didian

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Didian workspace.
...
A skill is installed for Didian only when it exists in the current workspace's
...
```bash
didian skill import --url <url> --output json              # hosted source
didian skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Didian workspace DB, so Didian cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `didian skill import --url <url> --output json`:

```bash
didian skill import --url clawhub.ai/owner/skill --output json
didian skill import --url skills.sh/owner/repo/skill --output json
didian skill import --url github.com/owner/repo --output json
didian skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
didian skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`didian skill import --file <path> --output json` imports a skill from a local
...
```bash
didian skill import --url <url> --output json
```
...
```bash
didian agent skills add <agent-id> --skill-ids <skill-id> --output json
didian agent skills list <agent-id> --output json
```

After the final `didian agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`didian agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`didian agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `didian skill import --url <url>` is equivalent to
...
didian skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
didian skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
didian skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
didian skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
didian skill get <skill-id> --output json
```
...
Incorrect (bypasses Didian):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Didian cannot manage it as a workspace skill.
...
```bash
didian skill import --url https://skills.sh/owner/repo/skill --output json
```
