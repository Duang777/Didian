...
// Mock @didian/views/navigation (AppLink + useNavigation)
vi.mock("../../navigation", () => ({
...
vi.mock("@didian/core/issues/stores/view-store", () => ({
...
vi.mock("@didian/core/issues/stores/view-store-context", () => ({
...
vi.mock("@didian/core/issues/stores/issues-scope-store", () => ({
...
vi.mock("@didian/core/issues/stores/selection-store", () => ({
...
vi.mock("@didian/core/issues/stores/recent-issues-store", () => ({
...
describe("IssuesPage (shared)", () => {
  ...
  it("shows the 'Missions' section header without a workspace prefix", async () => {
    mockListIssues.mockImplementation((params: any) =>
      Promise.resolve({
        issues: mockIssues.filter((i) => i.status === params?.status),
        total: mockIssues.filter((i) => i.status === params?.status).length,
  ...
  it("shows empty state when there are no issues", async () => {
    ...
    expect(screen.getByText("Create a Mission to get started.")).toBeInTheDocument();
  });
