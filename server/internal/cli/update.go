...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `didian_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("didian-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("didian_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/didian-ai/didian/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the didian GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/didian-ai/didian/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running didian binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade didian-ai/tap/didian`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "didian-ai/tap/didian")
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	tmpFile, err := os.CreateTemp(dir, "didian-update-*")
	if err != nil {
