---
title: 모바일 앱 (iOS)
description: 아직 App Store에 없는 오픈소스 Didian iOS 앱을 직접 본인 iPhone에 빌드하는 방법.
---

import { Callout } from "fumadocs-ui/components/callout";

Didian의 iOS 클라이언트는 오픈소스이며, web, desktop, 백엔드와 함께 [메인 저장소](https://github.com/didian-ai/didian)에 들어 있습니다. 아직 App Store에는 없으며, 그 상황이 바뀌기 전까지는 iPhone에서 사용하려는 사람이 직접 소스에서 빌드합니다. 빌드는 처음에는 약 10~20분, 그 이후에는 약 2분이 걸리며, [didian.ai](https://didian.ai)와 동일한 백엔드와 통신하므로 기존 계정이 그대로 동작합니다.

<Callout type="info">
이 페이지는 **개인 사용자**를 위한 것입니다. 앱 개발자는 저장소의 [`apps/mobile/README.md`](https://github.com/didian-ai/didian/blob/main/apps/mobile/README.md)를 읽어야 합니다 — dev / staging 변형과 전체 스크립트 목록을 다룹니다.
</Callout>
...
아직 자동 업데이트는 없습니다. Didian 코드베이스가 앞으로 나아가면, pull한 후 다시 빌드하세요:
...
iOS 앱은 여전히 빠르게 움직이고 있습니다 — 팀은 지금 App Store 심사 주기보다 출시 후 반복 개선을 선호합니다. 정식 App Store 출시 전에 TestFlight 베타가 가장 유력한 다음 단계입니다. 그때까지는 위의 직접 빌드 방식이 iOS에서 Didian를 사용하는 유일한 방법입니다.

TestFlight가 열릴 때 알림을 받고 싶다면 [GitHub 저장소](https://github.com/didian-ai/didian)를 watch하세요.

## 문제 해결

**"No matching provisioning profiles found"** — Xcode가 기본 번들 id `ai.didian.mobile`를 Apple ID로 서명하기를 거부합니다. 드물지만, 누군가 Apple 개발자 포털에서 해당 접두사를 등록했다면 발생합니다. 본인이 제어하는 아무 역방향 도메인(`com.yourname.didian`이면 충분합니다)을 골라 export한 후 다시 실행하세요:
...
**앱이 백엔드에 연결할 수 없음** — `apps/mobile/.env.production`이 수정되지 않았는지 확인하세요(기본값은 `EXPO_PUBLIC_API_URL=https://api.didian.ai`입니다). 변경했다면 `git checkout apps/mobile/.env.production`으로 복원하세요.
