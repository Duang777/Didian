...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("DIDIAN_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("DIDIAN_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("DIDIAN_APP_URL", "https://didian.internal.example/")
	...
	if cfg.DaemonServerURL != "https://didian.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://didian.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("DIDIAN_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://didian.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("DIDIAN_PUBLIC_URL", "https://api.didian.ai")
	t.Setenv("DIDIAN_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://didian.ai")
...
// frontend is didian.ai, but the deployment does not set DIDIAN_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://didian.ai, which the dialog turned into
// `didian setup self-host --server-url https://didian.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `didian setup`.
func TestGetConfigOmitsCloudDaemonSetupWithoutPublicURL(t *testing.T) {
	t.Setenv("DIDIAN_PUBLIC_URL", "")
	t.Setenv("DIDIAN_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://didian.ai")
...
// TestGetConfigOmitsCloudDaemonSetupForAppSubdomain covers the app.didian.ai
// frontend variant of the official cloud.
func TestGetConfigOmitsCloudDaemonSetupForAppSubdomain(t *testing.T) {
	t.Setenv("DIDIAN_PUBLIC_URL", "")
	t.Setenv("DIDIAN_APP_URL", "https://app.didian.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.didian.ai", want: true},
		{name: "bare host", raw: "api.didian.ai", want: true},
		{name: "host port", raw: "api.didian.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.didian.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.didian.ai"); got != tt.want {
