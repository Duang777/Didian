...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/didian_workspaces/.repos)
	logger *slog.Logger
...
// didianHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const didianHookMarker = "# didian:prepare-commit-msg:co-authored-by"
...
// treats a hook as Didian-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before didianHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	didianHookMarker,
	"# Installed by the Didian daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Didian Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# didian:prepare-commit-msg:co-authored-by
# Didian: add Co-authored-by trailer for the Didian Agent.
# Installed by the Didian daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: didian-agent <github@didian.ai>"
...
// a Co-authored-by trailer for the Didian Agent. The hook is installed in the
...
// installed by the Didian daemon (current or any previously released
