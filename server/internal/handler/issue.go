...
// translates the prompt into a `didian issue create` invocation in the
...
// the agent's `didian issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the didian CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See didian-ai/didian#1660.
