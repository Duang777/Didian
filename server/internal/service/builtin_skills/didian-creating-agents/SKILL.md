...
```bash
didian agent get <agent-id> --output json      # full persisted agent record
didian agent skills list <agent-id> --output json   # current skill bindings
didian agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
`POST /api/agents` (`didian agent create`). At task claim time the daemon
...
```bash
didian agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
didian agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`didian agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`didian agent env set`), which is owner/admin-only
...
```bash
didian agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
didian agent update <agent-id> --mcp-config-stdin --output json
didian agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `didian agent create` — inserts a new agent row.
- `didian agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `didian agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
