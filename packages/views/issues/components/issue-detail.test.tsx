...
import type { Issue, TimelineEntry } from "@didian/core/types";
...
// @didian/core/hooks.tsx). vi.mock("@didian/core/paths") only intercepts
...
// Mock @didian/core/workspace/hooks
...
// Mock @didian/core/paths — after the URL-driven workspace refactor,
...
// Mock issue config
vi.mock("@didian/core/issues/config", () => ({
...
vi.mock("@didian/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @didian/ui/components/ui/resizable)
...
describe("IssueDetail (shared)", () => {
  ...
  it("shows 'not found' message when issue does not exist", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("This Mission does not exist or has been deleted in this workspace."),
      ).toBeInTheDocument();
    });
  });

  it("shows 'Back to Missions' button when issue is not found and no onDelete prop", async () => {
    mockApiObj.getIssue.mockRejectedValue(new Error("Not found"));
