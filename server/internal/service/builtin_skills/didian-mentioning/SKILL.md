...
This skill states WHAT a mention link does in the Didian backend, traced to
...
- a person → `didian workspace member list --output json` → use `user_id`
- an agent → `didian agent list --output json` → use `id`
- a squad  → `didian squad list --output json` → use `id`
...
- **A private agent you cannot access:** skipped — the mention path gates on
  ...
  one; the child-done wake is ungated — see the didian-squads skill).
...
Correct:
  1. `didian workspace member list --output json`  → Alice's `user_id` = 7f3a…
