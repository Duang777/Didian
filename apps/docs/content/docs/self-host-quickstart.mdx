...
description: Run Didian on your own server or machine with Docker (or Helm on Kubernetes). Takes about 10 minutes.
---

import { Callout } from "fumadocs-ui/components/callout";

This page walks you through running the Didian **server** (backend + frontend + PostgreSQL) on your own machine or server with Docker. When you're done, your data is fully under your control — including [workspaces](/workspaces), [issues](/issues), [comments](/comments), and [agent](/agents) configuration.
...
2. Pull the official Docker images (PostgreSQL, Didian backend, Didian frontend)
...
**`docker-compose.selfhost.yml` sets `APP_ENV` to `production` by default** and leaves `DIDIAN_DEV_VERIFICATION_CODE` empty, so there is no fixed code on public instances.

Only set `DIDIAN_DEV_VERIFICATION_CODE` for local or private test automation. If a fixed code is enabled while `APP_ENV` is non-production, anyone who can request a code can sign in with that fixed value. See [Auth setup → Fixed local testing codes](/auth-setup#fixed-local-testing-codes).

Before any public deployment, make sure `.env` has `APP_ENV=production` and `DIDIAN_DEV_VERIFICATION_CODE` is empty.
</Callout>
...
SMTP_PORT=465
SMTP_USERNAME=didian@yourdomain.com
SMTP_PASSWORD=...
...
- Do not use `888888` unless you explicitly set `DIDIAN_DEV_VERIFICATION_CODE=888888` on a non-production private instance
- Log in and create your first workspace

## 5. Point the CLI at your own server

The CLI install is the same as in [Cloud quickstart → 2. Install the CLI](/cloud-quickstart#2-install-the-didian-cli) — Homebrew / script / PowerShell, pick one.
...
<Callout type="info">
Prefer environment variables over flags? `setup self-host` reads `DIDIAN_SERVER_URL` and `DIDIAN_APP_URL` when the matching flag is omitted — a flag still takes precedence over the env var. `DIDIAN_SERVER_URL` also accepts the `ws://…/ws` daemon form from [Environment variables](/environment-variables) and normalizes it to the HTTP base.
</Callout>
...
After bringing the proxy up, set `FRONTEND_ORIGIN=https://didian.example.com` in the server's `.env` and restart the backend — otherwise the WebSocket origin check will reject the browser ([Troubleshooting → WebSocket can't connect](/troubleshooting#websocket-cant-connect)).
...
Full reference — including operations notes and the Kubernetes deployment shape — lives in the repo's [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/didian-ai/didian/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).

## Kubernetes deployment (alternative)

If you already run a Kubernetes cluster, the repo also ships a Helm chart at `deploy/helm/didian/`. It's the equivalent of `make selfhost` for k8s — same backend image, frontend image, and `pgvector/pgvector:pg17` Postgres, packaged as Deployments / Services / Ingresses with one `ConfigMap` rendered from `values.yaml`. Authored against k3s + Traefik + `local-path` and should work on any cluster with an Ingress controller and a default `ReadWriteOnce` StorageClass.

The chart **does not template secret values**. It references a Secret named `didian-secrets` by name, so real JWT / DB / Resend / Google keys never need to live in git or in `values.yaml`. Create the namespace + Secret once with kubectl:
...
Defaults assume the hostnames `didian.dev.lan` (web) and `api.didian.dev.lan` (backend). Add them to `/etc/hosts` (or local DNS) pointing at any node IP where your Ingress is reachable. To use different hostnames, copy `deploy/helm/didian/values.yaml`, edit `ingress.frontend.host` / `ingress.backend.host` and the matching `backend.config.appUrl` / `frontendOrigin` / `localUploadBaseUrl` / `googleRedirectUri`, then install with `-f my-values.yaml`.
...
Then open `http://didian.dev.lan` and continue at [Step 4 — First login](#4-first-login--create-a-workspace) above. Point the CLI at your Ingress hostnames:
...
To pull the latest images without changing the chart, `kubectl -n didian rollout restart deploy/didian-backend deploy/didian-frontend`. To pin a specific Didian release, set `images.backend.tag` / `images.frontend.tag` in your values file and `helm upgrade`. `helm -n didian uninstall didian` removes the workloads but keeps the PVCs and Secret; `kubectl delete namespace didian` wipes everything.

The full reference — three login modes, the `backend` ExternalName workaround for the build-time-baked `REMOTE_API_URL` in the web image, resource limits, and TLS — lives in the repo's [`SELF_HOSTING.md`](https://github.com/didian-ai/didian/blob/main/SELF_HOSTING.md#kubernetes-deployment-alternative).
...
- [Desktop app](/desktop-app) — optional Desktop setup via `~/.didian/desktop.json`; the web frontend + CLI remains the quickest self-host path
